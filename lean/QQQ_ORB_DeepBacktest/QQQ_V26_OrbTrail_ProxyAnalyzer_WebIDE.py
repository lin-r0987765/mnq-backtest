...
class QQQV26OrbTrailProxyAnalyzerWebIDE(QCAlgorithm):
    """Analyzer-only v26 evaluator with ORB-range-denominated trailing stop.

    Structural change: replaces the fixed 1.3% trailing stop with
    trail_stop = best_price - TRAIL_ORB_MULT * orb_range.

    This normalises the one component in v26 that was NOT ORB-range-based:
    - Initial stop: already ORB range (orb_low / orb_high)
    - Breakeven: 1.25x ORB range
    - Profit-lock: 1.50x trigger / 0.25x floor — ORB range
    - Trailing stop: was 1.3% of price, NOW K × ORB range

    Local Alpaca result (best variant orb_trail_2.0x):
    - delta +8.74, 3/4 folds, 5/7 years, saved=48, clipped=15
    - verdict: READY_FOR_QC_PROXY
    """
    SYMBOL = "QQQ"
    SCRIPT_VERSION = "v26-orb-trail-qc-evaluator"
    INITIAL_CASH = 100000
    START = (2017, 4, 3)
    END = (2026, 4, 2)
    ...
    # trail_stop = best_price -/+ TRAIL_ORB_MULT * range_width
    TRAIL_ORB_MULT = 2.0
    # Original 1.3% trail is kept as a fallback reference but NOT used for stops:
    TRAILING_PCT = 0.013
    ...
    def initialize(self) -> None:
        ...
        self.orb_trail_stop_exit_count = 0
        ...
        self.debug(
            "QQQ v26 orb-trail evaluator init | "
            ...
            f"profit_ratio={self.PROFIT_RATIO} | trail_orb_mult={self.TRAIL_ORB_MULT} | "
    ...
    def on_end_of_algorithm(self) -> None:
        ...
        self.debug(
            "QQQ v26 orb-trail evaluator summary | "
            ...
            f"trail_orb_mult={self.TRAIL_ORB_MULT} | "
            f"orb_trail_stop_exits={self.orb_trail_stop_exit_count}"
        )
    ...
    def _manage_open_position(self, bar: TradeBar) -> None:
        ...
        # ORB-range-denominated trail distance
        trail_distance = self.TRAIL_ORB_MULT * self.range_width
        ...
        if holding.is_long:
            ...
            if (
                profit_lock_trigger_points > 0.0
            ...
            # ORB-range trail stop
            trail_stop = self.best_price_long - trail_distance
            ...
            if bar.close <= effective_stop:
                ...
                if is_profit_lock_stop:
                    ...
                    exit_tag = "ORB Long ORB Trail Stop"
                    self.orb_trail_stop_exit_count += 1
            ...
            # ORB-range trail stop
            trail_stop = self.best_price_short + trail_distance
            if self.breakeven_activated and gate_active:
            ...
            if bar.close >= effective_stop:
                ...
                if is_profit_lock_stop:
                    ...
                    exit_tag = "ORB Short ORB Trail Stop"
                    self.orb_trail_stop_exit_count += 1
                self.liquidate(self.symbol, exit_tag)
                self.exit_count += 1
                if is_profit_lock_stop:
