...
class QQQV26MidRatchetProxyAnalyzerWebIDE(QCAlgorithm):
    """Analyzer-only v26 evaluator with an added mid-trade protection ratchet."""
    SYMBOL = "QQQ"
    SCRIPT_VERSION = "v26-mid-ratchet-qc-evaluator"
    INITIAL_CASH = 100000
    START = (2017, 4, 3)
    END = (2026, 4, 2)
    ...
    MID_RATCHET_TRIGGER_MULT = 0.625
    MID_RATCHET_LEVEL_MULT = 0.50
    ...
    def initialize(self) -> None:
        ...
        self.mid_ratchet_activated = False
        self.mid_ratchet_price = None
        self.entry_count = 0
        self.exit_count = 0
        ...
        self.mid_ratchet_activation_count = 0
        self.mid_ratchet_stop_exit_count = 0
        ...
        self.debug(
            "QQQ v26 mid-ratchet evaluator init | "
            ...
            "baseline_reference=v26-profit-lock | research_only=1 | "
            ...
            f"mid_ratchet_trigger_mult={self.MID_RATCHET_TRIGGER_MULT} | "
            f"mid_ratchet_level_mult={self.MID_RATCHET_LEVEL_MULT} | "
    ...
    def on_end_of_algorithm(self) -> None:
        ...
        self.debug(
            "QQQ v26 mid-ratchet evaluator summary | "
            ...
            f"mid_ratchet_trigger_mult={self.MID_RATCHET_TRIGGER_MULT} | "
            f"mid_ratchet_level_mult={self.MID_RATCHET_LEVEL_MULT} | "
            f"mid_ratchet_activations={self.mid_ratchet_activation_count} | "
            f"mid_ratchet_stop_exits={self.mid_ratchet_stop_exit_count}"
        )
    ...
    def _manage_open_position(self, bar: TradeBar) -> None:
        ...
        mid_ratchet_trigger_points = self._mid_ratchet_trigger_points()
        mid_ratchet_level_points = self._mid_ratchet_level_points()
        ...
        if holding.is_long:
            ...
            if (
                mid_ratchet_trigger_points > 0.0
                and mid_ratchet_level_points > 0.0
                and not self.mid_ratchet_activated
                and bar.close - self.entry_price >= mid_ratchet_trigger_points
            ):
                self.mid_ratchet_activated = True
                self.mid_ratchet_price = self.entry_price + mid_ratchet_level_points
                self.mid_ratchet_activation_count += 1
            ...
            if self.mid_ratchet_activated and self.mid_ratchet_price is not None:
                effective_stop = max(effective_stop, self.mid_ratchet_price)
            if bar.close <= effective_stop:
                ...
                is_mid_ratchet_stop = (
                    ...
                    and self.mid_ratchet_activated
                    and self.mid_ratchet_price is not None
                    and self.mid_ratchet_price >= base_stop
                    and bar.close <= self.mid_ratchet_price * 1.001
                )
                is_be_stop = (
                    (not is_profit_lock_stop)
                    and (not is_mid_ratchet_stop)
                ...
                elif is_mid_ratchet_stop:
                    exit_tag = "ORB Long Mid Ratchet Stop"
                elif is_be_stop:
                ...
                if is_profit_lock_stop:
                    ...
                    self.mid_ratchet_stop_exit_count += 1
                elif is_be_stop:
            ...
            if (
                ...
                and mid_ratchet_level_points > 0.0
                and not self.mid_ratchet_activated
                and self.entry_price - bar.close >= mid_ratchet_trigger_points
            ):
                self.mid_ratchet_activated = True
                self.mid_ratchet_price = self.entry_price - mid_ratchet_level_points
                self.mid_ratchet_activation_count += 1
            ...
            if self.mid_ratchet_activated and self.mid_ratchet_price is not None:
                effective_stop = min(effective_stop, self.mid_ratchet_price)
            if bar.close >= effective_stop:
                is_profit_lock_stop = (
                    self.profit_lock_activated
                    and self.profit_lock_price is not None
                ...
                is_mid_ratchet_stop = (
                    ...
                    and self.mid_ratchet_price is not None
                    and self.mid_ratchet_price <= base_stop
                    and bar.close >= self.mid_ratchet_price * 0.999
                )
                is_be_stop = (
                    (not is_profit_lock_stop)
                    and (not is_mid_ratchet_stop)
                ...
                if is_profit_lock_stop:
                    ...
                    exit_tag = "ORB Short Mid Ratchet Stop"
                elif is_be_stop:
    ...
    def _mid_ratchet_trigger_points(self) -> float:
        if self.entry_price is None or self.MID_RATCHET_TRIGGER_MULT <= 0:
            return 0.0
        return self.MID_RATCHET_TRIGGER_MULT * self.entry_price * self.TRAILING_PCT

    def _mid_ratchet_level_points(self) -> float:
        if self.entry_price is None or self.MID_RATCHET_LEVEL_MULT <= 0:
            return 0.0
        return self.MID_RATCHET_LEVEL_MULT * self.entry_price * self.TRAILING_PCT
