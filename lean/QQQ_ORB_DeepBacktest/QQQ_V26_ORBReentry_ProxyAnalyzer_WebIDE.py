...
class QQQV26ORBReentryProxyAnalyzerWebIDE(QCAlgorithm):
    SYMBOL = "QQQ"
    ...
    def initialize(self) -> None:
        ...
        self.orb_reentry_armed = False
        self.orb_reentry_confirm_count = 0
        self.entry_count = 0
        self.exit_count = 0
        ...
        self.orb_reentry_armed_count = 0
        self.orb_reentry_exit_count = 0
        ...
        self.debug(
            "QQQ v26 ORB reentry evaluator init | "
            f"version={self.SCRIPT_VERSION} | baseline_reference=v26-profit-lock | research_only=1 | "
            f"security_type=equity | symbol={self.SYMBOL} | regime_mode={self.REGIME_MODE} | "
            f"trailing_pct={self.TRAILING_PCT} | breakeven_trigger_mult={self.BREAKEVEN_TRIGGER_MULT} | "
            ...
            f"orb_reentry_arm_progress_mult={self.ORB_REENTRY_ARM_PROGRESS_MULT} | "
            f"orb_reentry_depth_mult={self.ORB_REENTRY_DEPTH_MULT} | "
            f"orb_reentry_confirm_bars={self.ORB_REENTRY_CONFIRM_BARS} | "
            f"max_entries_per_session={self.MAX_ENTRIES_PER_SESSION} | "
    ...
    def on_end_of_algorithm(self) -> None:
        ...
        self.debug(
            "QQQ v26 ORB reentry evaluator summary | "
            f"version={self.SCRIPT_VERSION} | baseline_reference=v26-profit-lock | "
            f"entries={self.entry_count} | exits={self.exit_count} | total_shares={self.total_shares_traded} | "
            ...
            f"regime_history_shortage_days={self.regime_history_shortage_days} | reentry_blocked={self.reentry_blocked_count} | "
            ...
            f"orb_reentry_depth_mult={self.ORB_REENTRY_DEPTH_MULT} | "
            f"orb_reentry_confirm_bars={self.ORB_REENTRY_CONFIRM_BARS} | "
            f"orb_reentry_armed_count={self.orb_reentry_armed_count} | "
            f"orb_reentry_exit_count={self.orb_reentry_exit_count}"
        )
    ...
    def _start_new_session(self, bar: TradeBar) -> None:
        if self.current_session_open is not None and self.current_session_close is not None:
            self.up_day_streak = self.up_day_streak + 1 if self.current_session_close > self.current_session_open else 0
    ...
    def _update_regime_state(self) -> None:
        ...
        if len(close_s) < self.REGIME_MIN_HISTORY_DAYS:
            self.regime_allow_long_today = False
        ...
        prev_day_return = float(daily_ret.iloc[-1]) if pd.notna(daily_ret.iloc[-1]) else float("nan")
        mom3_return_s = close_s.pct_change(3)
        mom3_return = float(mom3_return_s.iloc[-1]) if pd.notna(mom3_return_s.iloc[-1]) else float("nan")
        ...
        self.regime_allow_long_today = prev_day_up and mom3_positive
        ...
        if self.regime_allow_long_today:
    ...
    def _manage_open_position(self, bar: TradeBar) -> None:
        ...
        if not holding.invested:
            self._reset_position_state()
            return
        if not holding.is_long or self.range_width is None or self.entry_price is None:
            return
        ...
        orb_reentry_arm_points = self._orb_reentry_arm_points()
        orb_reentry_depth_points = self._orb_reentry_depth_points()
        ...
        if (
            not self.orb_reentry_armed
            and orb_reentry_arm_points > 0.0
            and self.best_price_long - self.entry_price >= orb_reentry_arm_points
        ):
            self.orb_reentry_armed = True
            self.orb_reentry_armed_count += 1
        if self.orb_reentry_armed and self.orb_high is not None and orb_reentry_depth_points >= 0.0:
            reentry_level = self.orb_high - orb_reentry_depth_points
            if bar.close <= reentry_level:
                self.orb_reentry_confirm_count += 1
            else:
                self.orb_reentry_confirm_count = 0
            if self.orb_reentry_confirm_count >= self.ORB_REENTRY_CONFIRM_BARS:
                self.liquidate(self.symbol, "ORB Long ORB Reentry Exit")
                self.exit_count += 1
                self.orb_reentry_exit_count += 1
                self._reset_position_state()
                return
    ...
    def _orb_reentry_arm_points(self) -> float:
        if self.range_width is None or self.ORB_REENTRY_ARM_PROGRESS_MULT <= 0:
            return 0.0
        return self.ORB_REENTRY_ARM_PROGRESS_MULT * self.range_width

    def _orb_reentry_depth_points(self) -> float:
        if self.range_width is None or self.ORB_REENTRY_DEPTH_MULT < 0:
            return 0.0
        return self.ORB_REENTRY_DEPTH_MULT * self.range_width
