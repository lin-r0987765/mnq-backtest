...
CANDIDATES = [
    "close_top_quartile_20d_prev",
    "close_bottom_quartile_20d_prev",
    "close_midrange_20d_prev",
    "within_2pct_of_20d_high_prev",
    "within_2pct_of_20d_low_prev",
]
...
def build_feature_frame(daily_csv: Path) -> tuple[pd.DataFrame, list[object]]:
    ...
    range_width = (rolling_high_20 - rolling_low_20).replace(0.0, pd.NA)
    range_position_20 = (close - rolling_low_20) / range_width

    daily["prev_day_up"] = close.pct_change() > 0.0
    daily["mom3_positive"] = close.pct_change(3) > 0.0

    daily["close_top_quartile_20d_prev"] = range_position_20 >= 0.75
    daily["close_bottom_quartile_20d_prev"] = range_position_20 <= 0.25
    daily["close_midrange_20d_prev"] = (range_position_20 >= 0.35) & (range_position_20 <= 0.65)
    daily["within_2pct_of_20d_high_prev"] = close >= (rolling_high_20 * 0.98)
    daily["within_2pct_of_20d_low_prev"] = close <= (rolling_low_20 * 1.02)
...
def evaluate(
    ...
    return {
        "research_scope": "v18_range_location_bridge_family",
        "analysis_version": "v1_daily_rolling_range_location_and_overextension",
        ...
        "candidate_summary": {
            ...
            "interpretation": (
                "This family tests whether v18 benefits from prior-day rolling range location and overextension filters. "
                "It is intended as a fresh orthogonal branch after slow-trend, calmness/compression, displacement, "
                "volume, gap-context, and sequence-state families all failed."
            ),
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate daily rolling-range location / overextension bridge candidates on top of v18.")
    ...
    out_path = out_dir / "v18_range_location_bridge_family.json"
