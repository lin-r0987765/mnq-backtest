...
CANDIDATE_SPECS = {
    "move_ge_0p05_and_delay_ge_2bars": {"min_move_pct": 0.05, "min_delay_bars": 2},
    "move_ge_0p10_and_delay_ge_2bars": {"min_move_pct": 0.10, "min_delay_bars": 2},
    "move_ge_0p15_and_delay_ge_2bars": {"min_move_pct": 0.15, "min_delay_bars": 2},
    "move_ge_0p05_and_delay_ge_4bars": {"min_move_pct": 0.05, "min_delay_bars": 4},
    "move_ge_0p10_and_delay_ge_4bars": {"min_move_pct": 0.10, "min_delay_bars": 4},
    "move_ge_0p10_and_delay_1_to_4bars": {"min_move_pct": 0.10, "min_delay_bars": 1, "max_delay_bars": 4},
    "move_ge_0p10_and_delay_2_to_5bars": {"min_move_pct": 0.10, "min_delay_bars": 2, "max_delay_bars": 5},
}
...
def build_candidate_mask(frame: pd.DataFrame, spec: dict[str, float]) -> pd.Series:
    move = pd.to_numeric(frame["entry_move_from_open_pct"], errors="coerce")
    delay = pd.to_numeric(frame["post_orb_delay_bars"], errors="coerce")

    mask = move >= float(spec["min_move_pct"])
    if "min_delay_bars" in spec:
        mask &= delay >= float(spec["min_delay_bars"])
    if "max_delay_bars" in spec:
        mask &= delay <= float(spec["max_delay_bars"])
...
def evaluate(
    ...
    result = {
        "research_scope": "v18_intraday_execution_zone_bridge_family",
        "analysis_version": "v1_delayed_displacement_execution_zone",
        ...
        "candidate_summary": {
            ...
            "interpretation": (
                "This family tests same-day intraday ORB execution-quality zones by requiring both a minimum post-open "
                "displacement and a minimum number of bars after the ORB window has closed. It is intended to move "
                "beyond simple stretch thresholds, simple clock-phase windows, and simple delay-only gates."
            ),
        },
    }
    return result
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze v18 delayed-displacement intraday execution-zone candidates.")
    ...
    parser.add_argument("--output", default="results/qc_regime_prototypes/v18_intraday_execution_zone_bridge_family.json")
