#!/usr/bin/env python3
"""
Alpaca-backed local research for a structural ORB re-entry exit on top of v26.

Why this exists:
- v26-profit-lock is the official baseline.
- The main unresolved post-v26 weakness is no longer a generic late-session
  washout; it is the subset of trades that make moderate progress and then
  structurally fail before durable protection fully helps.
- Previously tested branches focused on timeouts, giveback thresholds, low
  progress, or simple stop ratchets.
- This branch is structurally different: once a breakout proves itself with
  enough progress, exit if price is later accepted back inside the original ORB.
...
- Add a new optional structural protection branch:
  - once the trade has achieved `arm_progress x ORB range` MFE,
  - watch for closes that re-enter the original ORB zone by at least
    `reentry_depth x ORB range`,
  - if that structural re-entry is confirmed for `confirm_bars` consecutive bars,
...
def simulate_orb_v26_orb_reentry_exit(
    df: pd.DataFrame,
    params: dict,
    *,
    arm_progress_mult: float | None,
    reentry_depth_mult: float | None,
    ...
    use_orb_reentry_exit = (
        arm_progress_mult is not None
        and arm_progress_mult > 0
        and reentry_depth_mult is not None
        and reentry_depth_mult >= 0
        and confirm_bars is not None
        and confirm_bars > 0
    )
    ...
    for _, session in df.groupby(df.index.date):
        ...
        arm_progress_pts = arm_progress_mult * range_width if use_orb_reentry_exit else None
        reentry_depth_pts = reentry_depth_mult * range_width if use_orb_reentry_exit else None
        ...
        for ts, row in post_orb.iterrows():
            ...
            if in_long:
                ...
                if (
                    use_orb_reentry_exit
                    and not orb_reentry_armed
                    and (best_price_long - entry_price) >= arm_progress_pts
                ):
                    orb_reentry_armed = True

                if use_orb_reentry_exit and orb_reentry_armed and reentry_depth_pts is not None:
...
def main() -> int:
    ...
    print("Loading local data for v26 ORB re-entry exit research...")
    ...
    for arm_progress in ARM_PROGRESS_MULTS:
        for reentry_depth in REENTRY_DEPTH_MULTS:
            for confirm_bars in CONFIRM_BARS_OPTIONS:
                label = (
                    f"orb_reentry_after_{arm_progress:.3f}x_"
                    f"depth_{reentry_depth:.2f}x_confirm_{confirm_bars}bar"
                )
                print(f"Running {label}...")
                variant_trades = simulate_orb_v26_orb_reentry_exit(
                    intraday,
                    BASE_PARAMS,
    ...
    payload = {
        "research_scope": "local_orb_v26_orb_reentry_exit_alpaca",
        ...
        "method": (
            "Alpaca-backed local feasibility test for a structural ORB re-entry exit on top of v26. "
            "Once a breakout has made enough favourable progress, the branch exits if price is later "
            "accepted back inside the original ORB for a configurable number of consecutive bars."
        ),
        "baseline": {
        ...
        "structural_conclusion": {
            "research_interpretation": (
                "This branch is structurally different from timeout, giveback, low-progress, and mid-ratchet lines "
                "because it treats breakout failure as a re-acceptance back into the original ORB zone."
            ),
            "next_step_rule": (
                "Do not launch a QC candidate unless the branch improves full-sample Alpaca PnL and shows at least 2/4 improved time folds and 3+ improved years."
            ),
        },
    }
