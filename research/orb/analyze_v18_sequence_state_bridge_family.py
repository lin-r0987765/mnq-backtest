...
CANDIDATES = [
    "first_green_after_red_prev",
    "second_green_after_red_prev",
    "up_streak_3_prev",
    "two_of_last_three_green_prev",
    "alternating_green_recovery_prev",
]
...
def build_feature_frame(daily_csv: Path) -> tuple[pd.DataFrame, list[object]]:
    ...
    positive_day = (ret1 > 0.0).astype(int)

    daily["prev_day_up"] = ret1 > 0.0
    daily["mom3_positive"] = close.pct_change(3) > 0.0

    daily["first_green_after_red_prev"] = (ret1 > 0.0) & (ret1.shift(1) <= 0.0)
    daily["second_green_after_red_prev"] = (
        (ret1 > 0.0) & (ret1.shift(1) > 0.0) & (ret1.shift(2) <= 0.0)
    )
    daily["up_streak_3_prev"] = (ret1 > 0.0) & (ret1.shift(1) > 0.0) & (ret1.shift(2) > 0.0)
    daily["two_of_last_three_green_prev"] = (
        positive_day.rolling(3, min_periods=3).sum() >= 2
    ) & (ret1 > 0.0)
    daily["alternating_green_recovery_prev"] = (
        (ret1 > 0.0) & (ret1.shift(1) <= 0.0) & (ret1.shift(2) > 0.0)
    )
...
def evaluate(
    ...
    return {
        "research_scope": "v18_sequence_state_bridge_family",
        "analysis_version": "v1_daily_sequence_state_and_sign_pattern",
        ...
        "candidate_summary": {
            ...
            "interpretation": (
                "This family tests whether v18 benefits from prior-day sequence-state and sign-pattern filters. "
                "It is intended as a fresh orthogonal branch after slow-trend, calmness/compression, displacement, "
                "volume, and gap-context families all failed."
            ),
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate daily sequence-state bridge candidates on top of v18.")
    ...
    out_path = out_dir / "v18_sequence_state_bridge_family.json"
