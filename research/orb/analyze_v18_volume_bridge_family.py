...
CANDIDATES = [
    "high_volume_prev",
    "low_volume_prev",
    "up_day_high_volume_prev",
    "up_day_low_volume_prev",
    "volume_expansion_vs_5d_prev",
]
...
def build_feature_frame(daily_csv: Path) -> tuple[pd.DataFrame, list[object]]:
    ...
    vol_sma5 = volume.rolling(5, min_periods=5).mean()
    vol_ratio = volume / vol_sma20.replace(0.0, pd.NA)

    daily["prev_day_up"] = ret1 > 0.0
    daily["mom3_positive"] = close.pct_change(3) > 0.0

    daily["high_volume_prev"] = vol_ratio >= 1.10
    daily["low_volume_prev"] = vol_ratio <= 0.90
    daily["up_day_high_volume_prev"] = (ret1 > 0.0) & (vol_ratio >= 1.10)
    daily["up_day_low_volume_prev"] = (ret1 > 0.0) & (vol_ratio <= 0.90)
    daily["volume_expansion_vs_5d_prev"] = volume > vol_sma5
...
def evaluate(
    ...
    return {
        "research_scope": "v18_volume_bridge_family",
        "analysis_version": "v1_daily_volume_and_participation",
        ...
        "candidate_summary": {
            ...
            "interpretation": (
                "This family tests whether v18 benefits from prior-day volume / participation filters. "
                "It is intended as a fresh orthogonal branch after adjacent slow-trend, calmness/compression, "
                "and displacement / candle-anatomy families all failed."
            ),
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate daily volume / participation bridge candidates on top of v18.")
    ...
    out_path = out_dir / "v18_volume_bridge_family.json"
