...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_sweep_lookback_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "sweep_lookback_30": {"overrides": {"liq_sweep_lookback": 30}},
        "sweep_lookback_40": {"overrides": {"liq_sweep_lookback": 40}},
        "sweep_lookback_60": {"overrides": {"liq_sweep_lookback": 60}},
        "sweep_lookback_70": {"overrides": {"liq_sweep_lookback": 70}},
        "sweep_lookback_90": {"overrides": {"liq_sweep_lookback": 90}},
    }
        ...
        ranked.append(
            {
                ...
                "liq_sweep_lookback": int(payload["params"]["liq_sweep_lookback"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate liquidity-sweep lookback on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "liq_sweep_lookback": int(params["liq_sweep_lookback"]),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_SWEEP_LOOKBACK_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby liquidity-sweep lookback changes do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "SWEEP_LOOKBACK_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested liquidity-sweep lookback variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_SWEEP_LOOKBACK_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one liquidity-sweep lookback calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_SWEEP_LOOKBACK_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A liquidity-sweep lookback calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_SWEEP_LOOKBACK_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive liquidity-sweep lookback calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_sweep_lookback_calibration",
        ...
        "best_robust_sweep_lookback_variant": best_robust_variant,
