...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_frontier_revisit_depth.json"
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "depth_0p75": {"overrides": {"fvg_revisit_depth_ratio": 0.75}},
        "depth_1p00": {"overrides": {"fvg_revisit_depth_ratio": 1.0}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_depth_filtered_retests": int(payload["metadata"]["fvg_depth_filtered_retests"]),
...
def _verdict(
    *,
    ...
    if best_robust_variant is not None:
        return (
            "ROBUST_LITE_FRONTIER_REVISIT_DEPTH_EXTENSION_IDENTIFIED",
            "At least one revisit-depth variant improves both activity and return versus the active lite frontier.",
        )
    if best_non_base is None:
        return (
            "LITE_FRONTIER_REVISIT_DEPTH_NO_POSITIVE_VARIANTS",
            "No positive revisit-depth variant survives on top of the active lite frontier.",
        )
    if best_positive_density_variant is not None and best_positive_density_variant["total_trades"] > base_trades:
        return (
            ...
            "A revisit-depth variant improves density on the active lite frontier, but not enough to replace the base cleanly.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_FRONTIER_REVISIT_DEPTH_SURVIVOR_BUT_NOT_EXTENSION",
            "Positive revisit-depth variants survive on the active lite frontier, but none improve it enough to promote.",
        )
    return (
        "LITE_FRONTIER_REVISIT_DEPTH_CALIBRATION_REJECTED",
        "The tested revisit-depth variants do not improve the active lite frontier.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run revisit-depth recalibration directly on the active lite ICT frontier."
    )
    ...
    output = {
        "analysis": "ict_lite_frontier_revisit_depth",
