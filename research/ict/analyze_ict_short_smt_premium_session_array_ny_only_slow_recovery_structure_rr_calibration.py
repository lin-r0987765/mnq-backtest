...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_rr_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "rr_3p0": {"overrides": {"take_profit_rr": 3.0}},
        "rr_3p5": {"overrides": {"take_profit_rr": 3.5}},
        "rr_4p5": {"overrides": {"take_profit_rr": 4.5}},
        "rr_5p0": {"overrides": {"take_profit_rr": 5.0}},
        "rr_6p0": {"overrides": {"take_profit_rr": 6.0}},
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate take-profit RR on top of the paired ICT structure-aware frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_RR_BASELINE"
    interpretation = (
        "The structure-aware frontier remains strongest at the current take-profit RR. Nearby RR changes do not beat the current base."
    )
    if all_variants_match_base:
        verdict = "RR_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested take-profit RR variants collapse to the same structure-aware frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_RR_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one take-profit RR calibration improves the structure-aware frontier while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_RR_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A take-profit RR calibration survives on top of the structure-aware frontier without collapsing activity, but it does not beat the base."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_RR_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive take-profit RR calibration exists on top of the structure-aware frontier, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_rr_calibration",
        ...
        "best_robust_rr_variant": best_robust_variant,
