...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_rejection_body_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_rejection_body_min_pct": 0.0}},
        "fvg_rejection_body_0p10": {"overrides": {"fvg_rejection_body_min_pct": 0.10}},
        "fvg_rejection_body_0p20": {"overrides": {"fvg_rejection_body_min_pct": 0.20}},
        "fvg_rejection_body_0p30": {"overrides": {"fvg_rejection_body_min_pct": 0.30}},
        "fvg_rejection_body_0p40": {"overrides": {"fvg_rejection_body_min_pct": 0.40}},
        "fvg_rejection_body_0p50": {"overrides": {"fvg_rejection_body_min_pct": 0.50}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_rejection_body_min_pct": float(payload["params"]["fvg_rejection_body_min_pct"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "ifvg_entries": int(payload["metadata"].get("ifvg_entries", 0)),
                "fvg_body_filtered_retests": int(payload["metadata"].get("fvg_body_filtered_retests", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG rejection body quality on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "fvg_rejection_body_min_pct": float(params["fvg_rejection_body_min_pct"]),
            ...
            "metadata": {
                ...
                "fvg_body_filtered_retests": int(signals.metadata.get("fvg_body_filtered_retests", 0)),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_REJECTION_BODY_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG rejection-body filters do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_REJECTION_BODY_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG rejection-body variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_REJECTION_BODY_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG rejection-body calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_FVG_REJECTION_BODY_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An FVG rejection-body calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_REJECTION_BODY_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG rejection-body calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_rejection_body_calibration",
        ...
        "best_robust_fvg_rejection_body_variant": best_robust_variant,
