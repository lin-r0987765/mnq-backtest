...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_displacement_range_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"displacement_range_atr_mult": 0.0}},
        "displacement_range_atr_0p50": {"overrides": {"displacement_range_atr_mult": 0.50}},
        "displacement_range_atr_0p75": {"overrides": {"displacement_range_atr_mult": 0.75}},
        "displacement_range_atr_1p00": {"overrides": {"displacement_range_atr_mult": 1.00}},
        "displacement_range_atr_1p25": {"overrides": {"displacement_range_atr_mult": 1.25}},
        "displacement_range_atr_1p50": {"overrides": {"displacement_range_atr_mult": 1.50}},
    }
        ...
        ranked.append(
            {
                ...
                "displacement_range_atr_mult": float(payload["params"]["displacement_range_atr_mult"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "ifvg_entries": int(payload["metadata"].get("ifvg_entries", 0)),
                "displacement_range_filtered_shifts": int(
                    payload["metadata"].get("displacement_range_filtered_shifts", 0)
                ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate displacement range-vs-ATR quality on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "displacement_range_atr_mult": float(params["displacement_range_atr_mult"]),
            ...
            "metadata": {
                ...
                "displacement_range_filtered_shifts": int(
                    signals.metadata.get("displacement_range_filtered_shifts", 0)
                ),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_DISPLACEMENT_RANGE_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby displacement range-vs-ATR variants do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "DISPLACEMENT_RANGE_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested displacement range-vs-ATR variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_DISPLACEMENT_RANGE_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one displacement range-vs-ATR calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_DISPLACEMENT_RANGE_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A displacement range-vs-ATR calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_DISPLACEMENT_RANGE_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive displacement range-vs-ATR calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_displacement_range_calibration",
        ...
        "best_robust_displacement_range_variant": best_robust_variant,
