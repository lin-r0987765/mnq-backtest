...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_shift_buffer_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"structure_shift_close_buffer_ratio": 0.0}},
        "shift_buffer_0p05": {"overrides": {"structure_shift_close_buffer_ratio": 0.05}},
        "shift_buffer_0p10": {"overrides": {"structure_shift_close_buffer_ratio": 0.10}},
        "shift_buffer_0p15": {"overrides": {"structure_shift_close_buffer_ratio": 0.15}},
        "shift_buffer_0p20": {"overrides": {"structure_shift_close_buffer_ratio": 0.20}},
        "shift_buffer_0p25": {"overrides": {"structure_shift_close_buffer_ratio": 0.25}},
    }
        ...
        ranked.append(
            {
                ...
                "structure_shift_close_buffer_ratio": float(payload["params"]["structure_shift_close_buffer_ratio"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "structure_buffer_filtered_shifts": int(
                    payload["metadata"].get("structure_buffer_filtered_shifts", 0)
                ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate structure-shift close-through buffer on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "structure_shift_close_buffer_ratio": float(params["structure_shift_close_buffer_ratio"]),
            ...
            "metadata": {
                ...
                "structure_buffer_filtered_shifts": int(
                    signals.metadata.get("structure_buffer_filtered_shifts", 0)
                ),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_SHIFT_BUFFER_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby structure-shift close-buffer variants do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "SHIFT_BUFFER_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested structure-shift close-buffer variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_SHIFT_BUFFER_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one structure-shift close-buffer calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
        ...
        interpretation = (
            "A structure-shift close-buffer calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_SHIFT_BUFFER_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive structure-shift close-buffer calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_shift_buffer_calibration",
        ...
        "best_robust_structure_shift_buffer_variant": best_robust_variant,
