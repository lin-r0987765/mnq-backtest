...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_gap_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "fvg_gap_0p0006": {"overrides": {"fvg_min_gap_pct": 0.0006}},
        "fvg_gap_0p0008": {"overrides": {"fvg_min_gap_pct": 0.0008}},
        "fvg_gap_0p0012": {"overrides": {"fvg_min_gap_pct": 0.0012}},
        "fvg_gap_0p0015": {"overrides": {"fvg_min_gap_pct": 0.0015}},
        "fvg_gap_0p0020": {"overrides": {"fvg_min_gap_pct": 0.0020}},
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG min-gap geometry on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_GAP_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG min-gap changes do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_GAP_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG min-gap variants collapse to the same structure-aware slow-recovery NY-only frontier result, which indicates a local plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_GAP_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG min-gap calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_FVG_GAP_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An FVG min-gap calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_GAP_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG min-gap calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_gap_calibration",
        ...
        "best_robust_fvg_gap_variant": best_robust_variant,
