...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_reclaim_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "reclaim_0p05": {"overrides": {"liq_sweep_reclaim_ratio": 0.05}},
        "reclaim_0p10": {"overrides": {"liq_sweep_reclaim_ratio": 0.10}},
        "reclaim_0p15": {"overrides": {"liq_sweep_reclaim_ratio": 0.15}},
        "reclaim_0p20": {"overrides": {"liq_sweep_reclaim_ratio": 0.20}},
        "reclaim_0p25": {"overrides": {"liq_sweep_reclaim_ratio": 0.25}},
        "reclaim_0p30": {"overrides": {"liq_sweep_reclaim_ratio": 0.30}},
    }
        ...
        ranked.append(
            {
                ...
                "liq_sweep_reclaim_ratio": float(payload["params"]["liq_sweep_reclaim_ratio"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate sweep reclaim strength on top of the paired ICT NY-only slow-recovery frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "liq_sweep_reclaim_ratio": float(params["liq_sweep_reclaim_ratio"]),
    ...
    verdict = "SLOW_RECOVERY_BASE_REMAINS_RECLAIM_FRONTIER"
    interpretation = (
        "The slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby reclaim-strength changes do not beat the current frontier."
    )
    if best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_RECLAIM_EXTENSION_IDENTIFIED_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "At least one reclaim-strength calibration improves the slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_RECLAIM_SURVIVOR_BUT_NOT_FRONTIER_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "A reclaim-strength calibration survives on top of the slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_RECLAIM_EXTENSION_ONLY_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "A positive reclaim-strength calibration exists on top of the slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_reclaim_calibration",
        ...
        "best_robust_reclaim_variant": best_robust_variant,
