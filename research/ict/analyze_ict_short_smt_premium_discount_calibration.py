...
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_short_smt_premium_discount_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "premium_tighter_band_0p03": {
            "overrides": {
                "use_premium_discount_filter": True,
                "premium_discount_neutral_band": 0.03,
            },
        },
        "premium_looser_band_0p08": {
            "overrides": {
                "use_premium_discount_filter": True,
                "premium_discount_neutral_band": 0.08,
            },
        },
        "premium_shorter_lookback_3": {
            "overrides": {
                "use_premium_discount_filter": True,
                "premium_discount_lookback": 3,
            },
        },
        "premium_longer_lookback_8": {
            "overrides": {
                "use_premium_discount_filter": True,
                "premium_discount_lookback": 8,
            },
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate premium/discount geometry on top of the paired ICT short-SMT premium base."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                ...
                "premium_discount_lookback": int(params["premium_discount_lookback"]),
    ...
    verdict = "PREMIUM_BASE_REMAINS_PREMIUM_DISCOUNT_FRONTIER"
    interpretation = (
        "The short-SMT premium paired ICT base remains the strongest robust profile. Nearby premium/discount geometry tweaks do not beat the current base."
    )
    if best_robust_premium and best_robust_premium["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_PREMIUM_DISCOUNT_EXTENSION_IDENTIFIED"
        interpretation = (
            "At least one premium/discount calibration improves the short-SMT premium base while preserving most of its activity. The next step should preserve that survivor before reopening additional context."
        )
    elif best_robust_premium and best_robust_premium["total_return_pct"] > 0:
        verdict = "ROBUST_PREMIUM_DISCOUNT_SURVIVOR_BUT_NOT_FRONTIER"
        interpretation = (
            "A premium/discount calibration survives on top of the short-SMT premium base without collapsing activity, but it does not beat the base itself. The next step should keep premium as the frontier and treat the tweak as a secondary optional branch."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_PREMIUM_DISCOUNT_EXTENSION_ONLY"
        interpretation = (
            "A positive premium/discount calibration exists on top of the short-SMT premium base, but only as a thinner continuation. The next step should treat it as optional refinement rather than a new frontier."
        )

    output = {
        "analysis": "ict_short_smt_premium_discount_calibration",
        ...
        "best_robust_premium_variant": best_robust_premium,
