...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_revisit_delay_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_revisit_min_delay_bars": 1}},
        "fvg_revisit_delay_2": {"overrides": {"fvg_revisit_min_delay_bars": 2}},
        "fvg_revisit_delay_3": {"overrides": {"fvg_revisit_min_delay_bars": 3}},
        "fvg_revisit_delay_4": {"overrides": {"fvg_revisit_min_delay_bars": 4}},
        "fvg_revisit_delay_5": {"overrides": {"fvg_revisit_min_delay_bars": 5}},
        "fvg_revisit_delay_6": {"overrides": {"fvg_revisit_min_delay_bars": 6}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_delay_filtered_retests": int(payload["metadata"].get("fvg_delay_filtered_retests", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG revisit delay on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_REVISIT_DELAY_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG revisit-delay variants do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_REVISIT_DELAY_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG revisit-delay variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
        ...
        interpretation = (
            "At least one FVG revisit-delay calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_FVG_REVISIT_DELAY_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An FVG revisit-delay calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_REVISIT_DELAY_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG revisit-delay calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_revisit_delay_calibration",
        ...
        "best_robust_fvg_revisit_delay_variant": best_robust_variant,
