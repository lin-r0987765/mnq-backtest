...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_session_array_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "broader_structural": {
            "overrides": {
                "structural_array_windows": ((10, 15, 12, 0), (12, 45, 15, 0)),
            },
        },
        "dual_broader": {
            "overrides": {
        ...
        "shifted_later": {
            "overrides": {
                ...
                "structural_array_windows": ((11, 0, 12, 0), (13, 30, 15, 0)),
            },
        },
    }
        ...
        ranked.append(
            {
                ...
                "use_session_array_refinement": bool(payload["params"]["use_session_array_refinement"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate session-array geometry on top of the paired ICT NY-only slow-recovery frontier."
    )
    ...
    verdict = "SLOW_RECOVERY_BASE_REMAINS_SESSION_ARRAY_FRONTIER"
    interpretation = (
        "The slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby session-array window changes do not beat the current frontier."
    )
    if best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_SESSION_ARRAY_EXTENSION_IDENTIFIED_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "At least one session-array calibration improves the slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_SESSION_ARRAY_SURVIVOR_BUT_NOT_FRONTIER_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "A session-array calibration survives on top of the slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_SESSION_ARRAY_EXTENSION_ONLY_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "A positive session-array calibration exists on top of the slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_session_array_calibration",
        ...
        "best_robust_session_array_variant": best_robust_variant,
