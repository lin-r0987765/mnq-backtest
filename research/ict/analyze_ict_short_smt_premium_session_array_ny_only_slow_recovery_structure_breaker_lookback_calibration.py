...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_breaker_lookback_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "breaker_lookback_8": {"overrides": {"breaker_lookback": 8}},
        "breaker_lookback_12": {"overrides": {"breaker_lookback": 12}},
        "breaker_lookback_20": {"overrides": {"breaker_lookback": 20}},
        "breaker_lookback_24": {"overrides": {"breaker_lookback": 24}},
        "breaker_lookback_30": {"overrides": {"breaker_lookback": 30}},
    }
        ...
        ranked.append(
            {
                ...
                "breaker_lookback": int(payload["params"]["breaker_lookback"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate breaker-block lookback on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "breaker_lookback": int(params["breaker_lookback"]),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_BREAKER_LOOKBACK_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby breaker-block lookback changes do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "BREAKER_LOOKBACK_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested breaker-block lookback variants collapse to the same structure-aware slow-recovery NY-only frontier result, which indicates a local plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_BREAKER_LOOKBACK_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one breaker-block lookback calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_BREAKER_LOOKBACK_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A breaker-block lookback calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_BREAKER_LOOKBACK_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive breaker-block lookback calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_breaker_lookback_calibration",
        ...
        "best_robust_breaker_lookback_variant": best_robust_variant,
