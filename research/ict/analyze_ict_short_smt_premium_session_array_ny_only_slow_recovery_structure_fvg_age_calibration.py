...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_age_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "fvg_age_10": {"overrides": {"fvg_max_age": 10}},
        "fvg_age_15": {"overrides": {"fvg_max_age": 15}},
        "fvg_age_25": {"overrides": {"fvg_max_age": 25}},
        "fvg_age_30": {"overrides": {"fvg_max_age": 30}},
        "fvg_age_40": {"overrides": {"fvg_max_age": 40}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_max_age": int(payload["params"]["fvg_max_age"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG max-age on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_AGE_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG max-age changes do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_AGE_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG max-age variants collapse to the same structure-aware slow-recovery NY-only frontier result, which indicates a local plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_AGE_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG max-age calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_FVG_AGE_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An FVG max-age calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_AGE_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG max-age calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_age_calibration",
        ...
        "best_robust_fvg_age_variant": best_robust_variant,
