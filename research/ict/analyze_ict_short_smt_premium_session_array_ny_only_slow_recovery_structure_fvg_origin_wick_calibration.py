...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_origin_wick_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_origin_opposite_wick_max_pct": 0.0}},
        "fvg_origin_wick_0p40": {"overrides": {"fvg_origin_opposite_wick_max_pct": 0.40}},
        "fvg_origin_wick_0p30": {"overrides": {"fvg_origin_opposite_wick_max_pct": 0.30}},
        "fvg_origin_wick_0p20": {"overrides": {"fvg_origin_opposite_wick_max_pct": 0.20}},
        "fvg_origin_wick_0p10": {"overrides": {"fvg_origin_opposite_wick_max_pct": 0.10}},
        "fvg_origin_wick_0p08": {"overrides": {"fvg_origin_opposite_wick_max_pct": 0.08}},
        "fvg_origin_wick_0p05": {"overrides": {"fvg_origin_opposite_wick_max_pct": 0.05}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_origin_opposite_wick_max_pct": float(payload["params"]["fvg_origin_opposite_wick_max_pct"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "ifvg_entries": int(payload["metadata"].get("ifvg_entries", 0)),
                "fvg_origin_wick_filtered_shifts": int(payload["metadata"].get("fvg_origin_wick_filtered_shifts", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG origin opposite-wick quality on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "fvg_origin_opposite_wick_max_pct": float(params["fvg_origin_opposite_wick_max_pct"]),
            ...
            "metadata": {
                ...
                "fvg_origin_wick_filtered_shifts": int(
                    signals.metadata.get("fvg_origin_wick_filtered_shifts", 0)
                ),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_ORIGIN_WICK_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG origin opposite-wick variants do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_ORIGIN_WICK_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG origin opposite-wick variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_ORIGIN_WICK_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG origin opposite-wick calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
        ...
        interpretation = (
            "An FVG origin opposite-wick calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_ORIGIN_WICK_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG origin opposite-wick calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_origin_wick_calibration",
        ...
        "best_robust_fvg_origin_wick_variant": best_robust_variant,
