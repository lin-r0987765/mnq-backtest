...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_revisit_depth_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_revisit_depth_ratio": 0.0}},
        "fvg_revisit_depth_0p25": {"overrides": {"fvg_revisit_depth_ratio": 0.25}},
        "fvg_revisit_depth_0p50": {"overrides": {"fvg_revisit_depth_ratio": 0.50}},
        "fvg_revisit_depth_0p75": {"overrides": {"fvg_revisit_depth_ratio": 0.75}},
        "fvg_revisit_depth_1p00": {"overrides": {"fvg_revisit_depth_ratio": 1.00}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_depth_filtered_retests": int(payload["metadata"].get("fvg_depth_filtered_retests", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG consequent-encroachment revisit depth on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_REVISIT_DEPTH_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG revisit-depth filters do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_REVISIT_DEPTH_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG revisit-depth variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_REVISIT_DEPTH_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG revisit-depth calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_FVG_REVISIT_DEPTH_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An FVG revisit-depth calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_REVISIT_DEPTH_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG revisit-depth calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_revisit_depth_calibration",
        ...
        "best_robust_fvg_revisit_depth_variant": best_robust_variant,
