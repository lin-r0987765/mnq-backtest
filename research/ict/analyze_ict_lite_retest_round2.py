...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_retest_round2.json"


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "lite_relaxed_smt_looser_sweep_base": {"overrides": {}},
        "depth_0p00": {"overrides": {"fvg_revisit_depth_ratio": 0.0}},
        "depth_0p25": {"overrides": {"fvg_revisit_depth_ratio": 0.25}},
        "delay_0": {"overrides": {"fvg_revisit_min_delay_bars": 0}},
        "delay_1": {"overrides": {"fvg_revisit_min_delay_bars": 1}},
        "delay_2": {"overrides": {"fvg_revisit_min_delay_bars": 2}},
        "displacement_0p00": {"overrides": {"displacement_body_min_pct": 0.0}},
        "displacement_0p05": {"overrides": {"displacement_body_min_pct": 0.05}},
        "displacement_0p15": {"overrides": {"displacement_body_min_pct": 0.15}},
    }
    ...
    *,
    ...
    if best_robust_variant is not None:
        return (
            ...
            "At least one retest-focused variant improves both activity and return versus the active lite geometry frontier.",
        )
    if best_non_base is None:
        return (
            "LITE_RETEST_NO_POSITIVE_VARIANTS",
            "No positive retest-focused variant survived on top of the active lite geometry frontier.",
        )
    if best_non_base["total_trades"] > base_trades and best_non_base["total_return_pct"] > 0:
        return (
            "LITE_RETEST_DENSITY_EXTENSION_ONLY",
            "A retest-focused variant improves activity on the active lite geometry frontier, but not enough to replace the base cleanly.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_RETEST_SURVIVOR_BUT_NOT_EXTENSION",
            "Positive retest-focused variants survive on top of the active lite geometry frontier, but none improve it enough to promote.",
        )
    return (
        "LITE_RETEST_CALIBRATION_REJECTED",
        "The tested retest-focused variants do not improve the active lite geometry frontier.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the second lite ICT retest-focused round on top of the relaxed-SMT looser-sweep frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        params = build_ict_lite_reversal_relaxed_smt_looser_sweep_profile_params(
            enable_smt=True,
    ...
    base_label = "lite_relaxed_smt_looser_sweep_base"
    ...
    output = {
        "analysis": "ict_lite_retest_round2",
