...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_ob_body_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "ob_body_0p20": {"overrides": {"ob_body_min_pct": 0.20}},
        "ob_body_0p25": {"overrides": {"ob_body_min_pct": 0.25}},
        "ob_body_0p35": {"overrides": {"ob_body_min_pct": 0.35}},
        "ob_body_0p40": {"overrides": {"ob_body_min_pct": 0.40}},
        "ob_body_0p50": {"overrides": {"ob_body_min_pct": 0.50}},
    }
        ...
        ranked.append(
            {
                ...
                "ob_body_min_pct": float(payload["params"]["ob_body_min_pct"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate order-block body quality on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "ob_body_min_pct": float(params["ob_body_min_pct"]),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_OB_BODY_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby order-block body-quality changes do not beat the current frontier."
    )
    if best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_OB_BODY_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one order-block body-quality calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_OB_BODY_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An order-block body-quality calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_OB_BODY_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive order-block body-quality calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_ob_body_calibration",
        ...
        "best_robust_ob_body_variant": best_robust_variant,
