...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_sweep_geometry_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "ny_only_base": {"overrides": {}},
        "sweep_shorter_lookback_35": {
            "overrides": {
                "liq_sweep_lookback": 35,
            },
        },
        "sweep_longer_lookback_70": {
            "overrides": {
                "liq_sweep_lookback": 70,
            },
        },
        "sweep_faster_recovery_2": {
            "overrides": {
                "liq_sweep_recovery_bars": 2,
            },
        },
        "sweep_slower_recovery_4": {
            "overrides": {
        ...
        ranked.append(
            {
                ...
                "liq_sweep_recovery_bars": int(payload["params"]["liq_sweep_recovery_bars"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate sweep geometry on top of the paired ICT NY-only short-SMT premium-plus-session-array frontier."
    )
    ...
    verdict = "NY_ONLY_BASE_REMAINS_SWEEP_GEOMETRY_FRONTIER"
    interpretation = (
        "The NY-only short-SMT premium-plus-session-array paired ICT base remains the strongest robust profile. Nearby sweep lookback and recovery-bar geometry changes do not beat the current frontier."
    )
    if best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_SWEEP_GEOMETRY_EXTENSION_IDENTIFIED_ON_NY_ONLY_BASE"
        interpretation = (
            "At least one sweep-geometry calibration improves the NY-only short-SMT premium-plus-session-array base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_SWEEP_GEOMETRY_SURVIVOR_BUT_NOT_FRONTIER_ON_NY_ONLY_BASE"
        interpretation = (
            "A sweep-geometry calibration survives on top of the NY-only short-SMT premium-plus-session-array base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_SWEEP_GEOMETRY_EXTENSION_ONLY_ON_NY_ONLY_BASE"
        interpretation = (
            "A positive sweep-geometry calibration exists on top of the NY-only short-SMT premium-plus-session-array base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_sweep_geometry_calibration",
        ...
        "best_robust_sweep_variant": best_robust_variant,
