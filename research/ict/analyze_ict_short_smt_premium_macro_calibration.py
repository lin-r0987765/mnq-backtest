...
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_short_smt_premium_macro_calibration.json"
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate macro timing windows on top of the paired ICT short-SMT premium base."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                ...
                "macro_windows": list(params["macro_windows"]) if "macro_windows" in params else None,
            },
    ...
    verdict = "PREMIUM_BASE_REMAINS_MACRO_FRONTIER"
    interpretation = (
        "The short-SMT premium paired ICT base remains the strongest robust profile. Macro timing still acts as a thinning filter rather than a robust extension."
    )
    if best_robust_macro and best_robust_macro["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_MACRO_EXTENSION_IDENTIFIED"
        interpretation = (
            "At least one macro timing calibration improves the short-SMT premium base while preserving most of its activity. The next step should preserve that macro survivor before reopening additional context."
        )
    elif best_robust_macro and best_robust_macro["total_return_pct"] > 0:
        verdict = "ROBUST_MACRO_SURVIVOR_BUT_NOT_FRONTIER"
        interpretation = (
            "A macro timing calibration survives on top of the short-SMT premium base without collapsing activity, but it does not beat the base itself. The next step should keep premium as the frontier and treat macro timing as a secondary optional branch."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_MACRO_EXTENSION_ONLY"
        interpretation = (
            "A positive macro timing calibration exists on top of the short-SMT premium base, but only as a thinner continuation. The next step should treat macro timing as optional refinement rather than a new frontier."
        )

    output = {
        "analysis": "ict_short_smt_premium_macro_calibration",
