...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_setup_density.json"


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "lite_faster_retest_base": {"overrides": {}},
        "setup_age_10": {"overrides": {"fvg_max_age": 10}},
        "setup_age_15": {"overrides": {"fvg_max_age": 15}},
        "setup_age_30": {"overrides": {"fvg_max_age": 30}},
        "setup_age_40": {"overrides": {"fvg_max_age": 40}},
        "setup_age_60": {"overrides": {"fvg_max_age": 60}},
    }
        ...
        ranked.append(
            {
                ...
                "bullish_shifts": int(payload["metadata"].get("bullish_shifts", 0)),
                "bearish_shifts": int(payload["metadata"].get("bearish_shifts", 0)),
            }
        )
    ranked.sort(
        key=lambda row: (
...
def _verdict(
    *,
    ...
    if best_robust_variant is not None:
        return (
            "ROBUST_LITE_SETUP_EXTENSION_IDENTIFIED",
            "At least one setup-density variant improves both activity and return versus the active lite faster-retest frontier.",
        )
    if best_positive_density_variant is not None:
        return (
            "LITE_SETUP_DENSITY_EXTENSION_ONLY",
            "A setup-density variant improves activity on the active lite faster-retest frontier, but not enough to replace the base cleanly.",
        )
    if best_non_base is None:
        return (
            "LITE_SETUP_NO_POSITIVE_VARIANTS",
            "No positive setup-density variant survived on top of the active lite faster-retest frontier.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_SETUP_SURVIVOR_BUT_NOT_EXTENSION",
            "Positive setup-density variants survive on top of the active lite faster-retest frontier, but none improve it enough to promote.",
        )
    return (
        "LITE_SETUP_CALIBRATION_REJECTED",
        "The tested setup-density variants do not improve the active lite faster-retest frontier.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the lite ICT setup-density round on top of the faster-retest lite frontier."
    )
    ...
    output = {
        "analysis": "ict_lite_setup_density",
