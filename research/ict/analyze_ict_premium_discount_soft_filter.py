...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_premium_discount_soft_filter.json"


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "strict_premium_hard_base": {"builder": "hard", "overrides": {}},
        "premium_band_0p15_hard": {
            "builder": "hard",
            "overrides": {"premium_discount_neutral_band": 0.15},
        },
        "premium_soft_penalty_0": {
            "builder": "soft",
            "overrides": {"premium_discount_mismatch_score_penalty": 0.0},
        },
        "premium_soft_penalty_2": {
            "builder": "soft",
            "overrides": {"premium_discount_mismatch_score_penalty": 2.0},
        },
        "premium_soft_penalty_3": {
            "builder": "soft",
            "overrides": {"premium_discount_mismatch_score_penalty": 3.0},
        },
        "premium_filter_off_control": {
            "builder": "hard",
            "overrides": {"use_premium_discount_filter": False},
        },
    }
...
def _build_params(builder_label: str, overrides: dict[str, Any]) -> dict[str, Any]:
    if builder_label == "soft":
        return build_ict_strict_soft_premium_profile_params(enable_smt=False, overrides=dict(overrides))
...
def _run_variant(merged, *, label: str, params: dict[str, Any], engine: BacktestEngine) -> dict[str, Any]:
    ...
    return {
        ...
        "params": {
            ...
            "premium_discount_neutral_band": float(params["premium_discount_neutral_band"]),
            "premium_discount_mismatch_score_penalty": float(
                params["premium_discount_mismatch_score_penalty"]
            ),
        ...
        "metadata": {
            ...
            "premium_discount_mismatch_setups": int(
                signals.metadata.get("premium_discount_mismatch_setups", 0)
            ),
            "premium_discount_softened_setups": int(
                signals.metadata.get("premium_discount_softened_setups", 0)
            ),
            "premium_discount_score_penalty_shifts": int(
                signals.metadata.get("premium_discount_score_penalty_shifts", 0)
            ),
...
def _rank_variants(results: dict[str, dict[str, Any]], base_trades: int) -> list[dict[str, Any]]:
    ...
    for label, payload in results.items():
        ...
        ranked.append(
            {
                "label": label,
                "premium_discount_filter_mode": payload["params"]["premium_discount_filter_mode"],
                "premium_discount_neutral_band": float(payload["params"]["premium_discount_neutral_band"]),
                "premium_discount_mismatch_score_penalty": float(
                    payload["params"]["premium_discount_mismatch_score_penalty"]
                ),
                ...
                "premium_discount_filtered_setups": int(
                    payload["metadata"]["premium_discount_filtered_setups"]
                ),
                "premium_discount_softened_setups": int(
                    payload["metadata"]["premium_discount_softened_setups"]
                ),
...
def _verdict(
    *,
    ...
    if best_robust_variant is not None:
        return (
            "ROBUST_PREMIUM_DISCOUNT_SOFT_FILTER_EXTENSION_IDENTIFIED",
            "A premium/discount softening variant improves both activity and return on the strict context lane.",
        )
    if best_non_base is None:
        return (
            "PREMIUM_DISCOUNT_SOFT_FILTER_NO_POSITIVE_VARIANTS",
            "No premium/discount softening variant survives on the strict context lane.",
        )
    if best_positive_density_variant is not None and best_positive_density_variant["total_trades"] > base_trades:
        return (
            "PREMIUM_DISCOUNT_SOFT_FILTER_DENSITY_EXTENSION_ONLY",
            "A premium/discount softening variant improves density, but not cleanly enough to replace the strict base.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "PREMIUM_DISCOUNT_SOFT_FILTER_SURVIVOR_BUT_NOT_EXTENSION",
            "A premium/discount softening variant stays positive, but none improve the strict base enough to promote.",
        )
    return (
        "PREMIUM_DISCOUNT_SOFT_FILTER_REJECTED",
        "Softening premium/discount does not produce a promotable improvement on the strict context lane.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Replay premium/discount soft-filter variants on the roadmap-aligned strict ICT lane."
    )
    ...
    base_label = "strict_premium_hard_base"
    ...
    output = {
        "analysis": "ict_premium_discount_soft_filter",
