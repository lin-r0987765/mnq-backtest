...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_atr_period_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "atr_period_10": {"overrides": {"atr_period": 10}},
        "atr_period_12": {"overrides": {"atr_period": 12}},
        "atr_period_16": {"overrides": {"atr_period": 16}},
        "atr_period_18": {"overrides": {"atr_period": 18}},
        "atr_period_20": {"overrides": {"atr_period": 20}},
    }
        ...
        ranked.append(
            {
                ...
                "atr_period": int(payload["params"]["atr_period"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate ATR period on top of the paired ICT structure-aware frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_ATR_PERIOD_BASELINE"
    interpretation = (
        "The structure-aware frontier remains strongest at the current ATR period. Nearby ATR period changes do not beat the current base."
    )
    if all_variants_match_base:
        verdict = "ATR_PERIOD_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested ATR period variants collapse to the same structure-aware frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_ATR_PERIOD_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one ATR period calibration improves the structure-aware frontier while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_ATR_PERIOD_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An ATR period calibration survives on top of the structure-aware frontier without collapsing activity, but it does not beat the base."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_ATR_PERIOD_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive ATR period calibration exists on top of the structure-aware frontier, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_atr_period_calibration",
        ...
        "best_robust_atr_period_variant": best_robust_variant,
