...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_touch_cap_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_max_retest_touches": 0}},
        "fvg_touch_cap_1": {"overrides": {"fvg_max_retest_touches": 1}},
        "fvg_touch_cap_2": {"overrides": {"fvg_max_retest_touches": 2}},
        "fvg_touch_cap_3": {"overrides": {"fvg_max_retest_touches": 3}},
        "fvg_touch_cap_4": {"overrides": {"fvg_max_retest_touches": 4}},
        "fvg_touch_cap_5": {"overrides": {"fvg_max_retest_touches": 5}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_max_retest_touches": int(payload["params"]["fvg_max_retest_touches"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "fvg_touch_filtered_retests": int(payload["metadata"].get("fvg_touch_filtered_retests", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG retest touch cap on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "fvg_max_retest_touches": int(params["fvg_max_retest_touches"]),
            ...
            "metadata": {
                ...
                "fvg_touch_filtered_retests": int(signals.metadata.get("fvg_touch_filtered_retests", 0)),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_TOUCH_CAP_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG touch-cap variants do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_TOUCH_CAP_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG touch-cap variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_TOUCH_CAP_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG touch-cap calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
        ...
        interpretation = (
            "An FVG touch-cap calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_TOUCH_CAP_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG touch-cap calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_touch_cap_calibration",
        ...
        "best_robust_fvg_touch_cap_variant": best_robust_variant,
