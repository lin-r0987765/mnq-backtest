...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_session_density.json"


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "lite_faster_retest_base": {"overrides": {}},
        "session_off_control": {"overrides": {"trade_sessions": False}},
        "ny_13_20": {"overrides": {"trade_sessions": True, "london_open": 0, "london_close": 0, "ny_open": 13, "ny_close": 20}},
        "ny_14_21": {"overrides": {"trade_sessions": True, "london_open": 0, "london_close": 0, "ny_open": 14, "ny_close": 21}},
        "ny_13_21": {"overrides": {"trade_sessions": True, "london_open": 0, "london_close": 0, "ny_open": 13, "ny_close": 21}},
        "broader_sessions": {"overrides": {"trade_sessions": True, "london_open": 8, "london_close": 16, "ny_open": 13, "ny_close": 21}},
        "london_ny_overlap": {"overrides": {"trade_sessions": True, "london_open": 12, "london_close": 17, "ny_open": 13, "ny_close": 21}},
    }
...
def _rank_variants(results: dict[str, dict[str, Any]], base_trades: int) -> list[dict[str, Any]]:
    ...
    for label, payload in results.items():
        ...
        ranked.append(
            {
                "label": label,
                "trade_sessions": bool(payload["params"]["trade_sessions"]),
                "london_open": int(payload["params"]["london_open"]),
                "london_close": int(payload["params"]["london_close"]),
                "ny_open": int(payload["params"]["ny_open"]),
                "ny_close": int(payload["params"]["ny_close"]),
...
def _verdict(
    *,
    ...
    if best_robust_variant is not None:
        return (
            "ROBUST_LITE_SESSION_EXTENSION_IDENTIFIED",
            "At least one session-density variant improves both activity and return versus the active lite retest frontier.",
        )
    if best_positive_density_variant is not None:
        return (
            "LITE_SESSION_DENSITY_EXTENSION_ONLY",
            "A session-density variant improves activity on the active lite retest frontier, but not enough to replace the base cleanly.",
        )
    if best_non_base is None:
        return (
            "LITE_SESSION_NO_POSITIVE_VARIANTS",
            "No positive session-density variant survived on top of the active lite retest frontier.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_SESSION_SURVIVOR_BUT_NOT_EXTENSION",
            "Positive session-density variants survive on top of the active lite retest frontier, but none improve it enough to promote.",
        )
    return (
        "LITE_SESSION_CALIBRATION_REJECTED",
        "The tested session-density variants do not improve the active lite retest frontier.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the lite ICT session-density round on top of the faster-retest lite frontier."
    )
    ...
    base_label = "lite_faster_retest_base"
    ...
    output = {
        "analysis": "ict_lite_session_density",
