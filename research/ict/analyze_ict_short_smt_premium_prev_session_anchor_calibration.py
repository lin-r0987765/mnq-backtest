...
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_short_smt_premium_prev_session_anchor_calibration.json"
)
    ...
    return {
        ...
        "anchor_tighter_0p03": {
            "overrides": {
                ...
                "prev_session_anchor_tolerance": 0.03,
            },
        },
        "anchor_default_0p05": {
            "overrides": {
        ...
        "anchor_looser_0p08": {
            "overrides": {
                "use_prev_session_anchor_filter": True,
                "prev_session_anchor_tolerance": 0.08,
            },
        },
        "anchor_looser_0p12": {
            "overrides": {
                "use_prev_session_anchor_filter": True,
                "prev_session_anchor_tolerance": 0.12,
            },
        },
    }
        ...
        ranked.append(
            {
                ...
                "prev_session_anchor_long_bars": int(payload["metadata"].get("prev_session_anchor_long_bars", 0)),
                "prev_session_anchor_short_bars": int(payload["metadata"].get("prev_session_anchor_short_bars", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate previous-session anchor behavior on top of the paired ICT short-SMT premium base."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            ...
            "metadata": {
                ...
                "prev_session_anchor_long_bars": int(signals.metadata.get("prev_session_anchor_long_bars", 0)),
                "prev_session_anchor_short_bars": int(signals.metadata.get("prev_session_anchor_short_bars", 0)),
    ...
    verdict = "PREMIUM_BASE_REMAINS_PREV_SESSION_ANCHOR_FRONTIER"
    interpretation = (
        "The short-SMT premium paired ICT base remains the strongest robust profile. Previous-session anchor tweaks do not beat the current base."
    )
    if best_robust_anchor and best_robust_anchor["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_PREV_SESSION_ANCHOR_EXTENSION_IDENTIFIED"
        interpretation = (
            "At least one previous-session-anchor calibration improves the short-SMT premium base while preserving most of its activity. The next step should preserve that survivor before reopening additional context."
        )
    elif best_robust_anchor and best_robust_anchor["total_return_pct"] > 0:
        verdict = "ROBUST_PREV_SESSION_ANCHOR_SURVIVOR_BUT_NOT_FRONTIER"
        interpretation = (
            "A previous-session-anchor calibration survives on top of the short-SMT premium base without collapsing activity, but it does not beat the base itself. The next step should keep premium as the frontier and treat the tweak as a secondary optional branch."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_PREV_SESSION_ANCHOR_EXTENSION_ONLY"
        interpretation = (
            "A positive previous-session-anchor calibration exists on top of the short-SMT premium base, but only as a thinner continuation. The next step should treat it as optional refinement rather than a new frontier."
        )

    output = {
        "analysis": "ict_short_smt_premium_prev_session_anchor_calibration",
        ...
        "best_robust_prev_session_anchor_variant": best_robust_anchor,
