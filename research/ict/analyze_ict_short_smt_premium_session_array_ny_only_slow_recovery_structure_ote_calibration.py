...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_ote_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "ote_off_control": {"overrides": {"score_ote_zone": 0}},
        "ote_tighter_band": {"overrides": {"ote_fib_low": 0.66, "ote_fib_high": 0.79}},
        "ote_looser_band": {"overrides": {"ote_fib_low": 0.55, "ote_fib_high": 0.82}},
        "ote_higher_score": {"overrides": {"score_ote_zone": 3}},
        "ote_lower_score": {"overrides": {"score_ote_zone": 1}},
        "ote_tighter_higher_score": {
            "overrides": {"ote_fib_low": 0.66, "ote_fib_high": 0.79, "score_ote_zone": 3}
        },
    }
        ...
        ranked.append(
            {
                ...
                "ote_fib_low": float(payload["params"]["ote_fib_low"]),
                "ote_fib_high": float(payload["params"]["ote_fib_high"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate OTE geometry and score on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_OTE_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby OTE geometry or score changes do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "OTE_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested OTE geometry and score variants collapse to the same structure-aware slow-recovery NY-only frontier result, which indicates a local plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_OTE_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one OTE geometry or score calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_OTE_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An OTE geometry or score calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_OTE_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive OTE calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_ote_calibration",
        ...
        "best_robust_ote_variant": best_robust_variant,
