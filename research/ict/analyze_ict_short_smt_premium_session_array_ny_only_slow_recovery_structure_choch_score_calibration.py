...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_choch_score_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "choch_score_0": {"overrides": {"score_choch": 0.0}},
        "choch_score_1": {"overrides": {"score_choch": 1.0}},
        "choch_score_2": {"overrides": {"score_choch": 2.0}},
        "choch_score_4": {"overrides": {"score_choch": 4.0}},
        "choch_score_5": {"overrides": {"score_choch": 5.0}},
        "choch_score_6": {"overrides": {"score_choch": 6.0}},
    }
        ...
        ranked.append(
            {
                ...
                "score_choch": float(payload["params"]["score_choch"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate CHOCH score on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "score_choch": float(params["score_choch"]),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_CHOCH_SCORE_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby CHOCH score changes do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "CHOCH_SCORE_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested CHOCH score variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_CHOCH_SCORE_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one CHOCH score calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_CHOCH_SCORE_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A CHOCH score calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_CHOCH_SCORE_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive CHOCH score calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_choch_score_calibration",
        ...
        "best_robust_choch_score_variant": best_robust_variant,
