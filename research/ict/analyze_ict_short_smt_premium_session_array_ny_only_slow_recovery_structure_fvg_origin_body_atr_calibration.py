...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_origin_body_atr_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_origin_body_atr_mult": 0.0}},
        "fvg_origin_body_atr_0p50": {"overrides": {"fvg_origin_body_atr_mult": 0.50}},
        "fvg_origin_body_atr_0p75": {"overrides": {"fvg_origin_body_atr_mult": 0.75}},
        "fvg_origin_body_atr_1p00": {"overrides": {"fvg_origin_body_atr_mult": 1.00}},
        "fvg_origin_body_atr_1p10": {"overrides": {"fvg_origin_body_atr_mult": 1.10}},
        "fvg_origin_body_atr_1p25": {"overrides": {"fvg_origin_body_atr_mult": 1.25}},
        "fvg_origin_body_atr_1p50": {"overrides": {"fvg_origin_body_atr_mult": 1.50}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_origin_body_atr_mult": float(payload["params"]["fvg_origin_body_atr_mult"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "ifvg_entries": int(payload["metadata"].get("ifvg_entries", 0)),
                "fvg_origin_body_atr_filtered_shifts": int(
                    payload["metadata"].get("fvg_origin_body_atr_filtered_shifts", 0)
                ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG origin body-vs-ATR quality on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "fvg_origin_body_atr_mult": float(params["fvg_origin_body_atr_mult"]),
            ...
            "metadata": {
                ...
                "fvg_origin_body_atr_filtered_shifts": int(
                    signals.metadata.get("fvg_origin_body_atr_filtered_shifts", 0)
                ),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_ORIGIN_BODY_ATR_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG origin body-vs-ATR variants do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_ORIGIN_BODY_ATR_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG origin body-vs-ATR variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_ORIGIN_BODY_ATR_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG origin body-vs-ATR calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
        ...
        interpretation = (
            "An FVG origin body-vs-ATR calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_ORIGIN_BODY_ATR_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG origin body-vs-ATR calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_origin_body_atr_calibration",
        ...
        "best_robust_fvg_origin_body_atr_variant": best_robust_variant,
