...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_frontier_fvg_gap.json"
...
def _verdict(
    *,
    ...
    if best_robust_variant is not None:
        return (
            "ROBUST_LITE_FRONTIER_FVG_GAP_EXTENSION_IDENTIFIED",
            "At least one FVG-gap variant improves both activity and return versus the active lite frontier.",
        )
    if best_non_base is None:
        return (
            "LITE_FRONTIER_FVG_GAP_NO_POSITIVE_VARIANTS",
            "No positive FVG-gap variant survives on top of the active lite frontier.",
        )
    if best_positive_density_variant is not None and best_positive_density_variant["total_trades"] > base_trades:
        return (
            "LITE_FRONTIER_FVG_GAP_DENSITY_EXTENSION_ONLY",
            "An FVG-gap variant improves density on the active lite frontier, but not enough to replace the base cleanly.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_FRONTIER_FVG_GAP_SURVIVOR_BUT_NOT_EXTENSION",
            "Positive FVG-gap variants survive on the active lite frontier, but none improve it enough to promote.",
        )
    return (
        "LITE_FRONTIER_FVG_GAP_CALIBRATION_REJECTED",
        "The tested FVG-gap variants do not improve the active lite frontier.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run FVG-gap recalibration directly on the active lite ICT frontier."
    )
    ...
    output = {
        "analysis": "ict_lite_frontier_fvg_gap",
