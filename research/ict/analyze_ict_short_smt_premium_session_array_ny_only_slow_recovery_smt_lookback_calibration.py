...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_smt_lookback_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "smt_lookback_8": {"overrides": {"smt_lookback": 8}},
        "smt_lookback_12": {"overrides": {"smt_lookback": 12}},
        "smt_lookback_6": {"overrides": {"smt_lookback": 6}},
        "smt_lookback_14": {"overrides": {"smt_lookback": 14}},
        "smt_off_control": {"overrides": {"use_smt_filter": False}},
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate SMT lookback on top of the paired ICT NY-only slow-recovery frontier."
    )
    ...
    verdict = "SLOW_RECOVERY_BASE_REMAINS_SMT_LOOKBACK_FRONTIER"
    interpretation = (
        "The slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby SMT lookback changes do not beat the current frontier."
    )
    if best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_SMT_LOOKBACK_EXTENSION_IDENTIFIED_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "At least one SMT lookback calibration improves the slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_SMT_LOOKBACK_SURVIVOR_BUT_NOT_FRONTIER_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "An SMT lookback calibration survives on top of the slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_SMT_LOOKBACK_EXTENSION_ONLY_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "A positive SMT lookback calibration exists on top of the slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_smt_lookback_calibration",
        ...
        "best_robust_smt_variant": best_robust_variant,
