...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_liquidity_pool_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "liq_pool_lookback_30": {"overrides": {"liq_sweep_lookback": 30}},
        "liq_pool_lookback_40": {"overrides": {"liq_sweep_lookback": 40}},
        "liq_pool_lookback_60": {"overrides": {"liq_sweep_lookback": 60}},
        "liq_pool_lookback_80": {"overrides": {"liq_sweep_lookback": 80}},
        "liq_pool_lookback_100": {"overrides": {"liq_sweep_lookback": 100}},
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate liquidity-pool lookback on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_LIQUIDITY_POOL_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby liquidity-pool lookback changes do not beat the current frontier."
    )
    if best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_LIQUIDITY_POOL_EXTENSION_IDENTIFIED_ON_STRUCTURE_FRONTIER"
        interpretation = (
            "At least one liquidity-pool lookback calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_LIQUIDITY_POOL_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A liquidity-pool lookback calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_LIQUIDITY_POOL_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive liquidity-pool lookback calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_liquidity_pool_calibration",
        ...
        "best_robust_liquidity_pool_variant": best_robust_variant,
