...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_displacement_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "displacement_0p10": {"overrides": {"displacement_body_min_pct": 0.10}},
        "displacement_0p20": {"overrides": {"displacement_body_min_pct": 0.20}},
        "displacement_0p30": {"overrides": {"displacement_body_min_pct": 0.30}},
        "displacement_0p40": {"overrides": {"displacement_body_min_pct": 0.40}},
        "displacement_0p50": {"overrides": {"displacement_body_min_pct": 0.50}},
        "displacement_0p60": {"overrides": {"displacement_body_min_pct": 0.60}},
    }
        ...
        ranked.append(
            {
                ...
                "displacement_body_min_pct": float(payload["params"]["displacement_body_min_pct"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "displacement_filtered_shifts": int(payload["metadata"].get("displacement_filtered_shifts", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate displacement body-ratio quality on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            ...
            "metadata": {
                ...
                "displacement_filtered_shifts": int(signals.metadata.get("displacement_filtered_shifts", 0)),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_DISPLACEMENT_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby displacement-body filters do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "DISPLACEMENT_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested displacement body-ratio variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_DISPLACEMENT_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one displacement body-ratio calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_DISPLACEMENT_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A displacement body-ratio calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_DISPLACEMENT_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive displacement body-ratio calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_displacement_calibration",
        ...
        "best_robust_displacement_variant": best_robust_variant,
