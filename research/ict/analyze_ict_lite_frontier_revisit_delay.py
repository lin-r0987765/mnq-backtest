...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_frontier_revisit_delay.json"
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "delay_3": {"overrides": {"fvg_revisit_min_delay_bars": 3}},
        "delay_4": {"overrides": {"fvg_revisit_min_delay_bars": 4}},
        "delay_5": {"overrides": {"fvg_revisit_min_delay_bars": 5}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_delay_filtered_retests": int(payload["metadata"]["fvg_delay_filtered_retests"]),
...
def _verdict(
    *,
    ...
    if best_robust_variant is not None:
        return (
            "ROBUST_LITE_FRONTIER_REVISIT_DELAY_EXTENSION_IDENTIFIED",
            "At least one revisit-delay variant improves both activity and return versus the active lite frontier.",
        )
    if best_non_base is None:
        return (
            "LITE_FRONTIER_REVISIT_DELAY_NO_POSITIVE_VARIANTS",
            "No positive revisit-delay variant survives on top of the active lite frontier.",
        )
    if best_positive_density_variant is not None and best_positive_density_variant["total_trades"] > base_trades:
        return (
            "LITE_FRONTIER_REVISIT_DELAY_DENSITY_EXTENSION_ONLY",
            "A revisit-delay variant improves density on the active lite frontier, but not enough to replace the base cleanly.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_FRONTIER_REVISIT_DELAY_SURVIVOR_BUT_NOT_EXTENSION",
            "Positive revisit-delay variants survive on the active lite frontier, but none improve it enough to promote.",
        )
    return (
        "LITE_FRONTIER_REVISIT_DELAY_CALIBRATION_REJECTED",
        "The tested revisit-delay variants do not improve the active lite frontier.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run revisit-delay recalibration directly on the active lite ICT frontier."
    )
    ...
    output = {
        "analysis": "ict_lite_frontier_revisit_delay",
