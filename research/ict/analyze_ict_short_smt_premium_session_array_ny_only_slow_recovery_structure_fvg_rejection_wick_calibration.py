...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_rejection_wick_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_rejection_wick_ratio": 0.0}},
        "fvg_rejection_wick_0p10": {"overrides": {"fvg_rejection_wick_ratio": 0.10}},
        "fvg_rejection_wick_0p20": {"overrides": {"fvg_rejection_wick_ratio": 0.20}},
        "fvg_rejection_wick_0p30": {"overrides": {"fvg_rejection_wick_ratio": 0.30}},
        "fvg_rejection_wick_0p40": {"overrides": {"fvg_rejection_wick_ratio": 0.40}},
        "fvg_rejection_wick_0p50": {"overrides": {"fvg_rejection_wick_ratio": 0.50}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_rejection_wick_ratio": float(payload["params"]["fvg_rejection_wick_ratio"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "ifvg_entries": int(payload["metadata"].get("ifvg_entries", 0)),
                "fvg_wick_filtered_retests": int(payload["metadata"].get("fvg_wick_filtered_retests", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG rejection wick strength on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "fvg_rejection_wick_ratio": float(params["fvg_rejection_wick_ratio"]),
            ...
            "metadata": {
                ...
                "fvg_wick_filtered_retests": int(signals.metadata.get("fvg_wick_filtered_retests", 0)),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_REJECTION_WICK_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG rejection-wick filters do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_REJECTION_WICK_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG rejection-wick variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_REJECTION_WICK_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG rejection-wick calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_FVG_REJECTION_WICK_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An FVG rejection-wick calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_REJECTION_WICK_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG rejection-wick calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_rejection_wick_calibration",
        ...
        "best_robust_fvg_rejection_wick_variant": best_robust_variant,
