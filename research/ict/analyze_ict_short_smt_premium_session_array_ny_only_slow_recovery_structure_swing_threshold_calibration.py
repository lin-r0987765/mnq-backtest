...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_swing_threshold_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "swing_threshold_2": {"overrides": {"swing_threshold": 2}},
        "swing_threshold_4": {"overrides": {"swing_threshold": 4}},
        "swing_threshold_5": {"overrides": {"swing_threshold": 5}},
        "swing_threshold_6": {"overrides": {"swing_threshold": 6}},
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate swing-threshold geometry on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_SWING_THRESHOLD_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby swing-threshold changes do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "SWING_THRESHOLD_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested swing-threshold variants collapse to the same structure-aware slow-recovery NY-only frontier result, which indicates a local plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_SWING_THRESHOLD_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one swing-threshold calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_SWING_THRESHOLD_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A swing-threshold calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_SWING_THRESHOLD_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive swing-threshold calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_swing_threshold_calibration",
        ...
        "best_robust_swing_threshold_variant": best_robust_variant,
