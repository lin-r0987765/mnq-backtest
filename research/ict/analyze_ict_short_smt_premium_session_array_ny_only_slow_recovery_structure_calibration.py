...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "structure_12": {"overrides": {"structure_lookback": 12}},
        "structure_16": {"overrides": {"structure_lookback": 16}},
        "structure_24": {"overrides": {"structure_lookback": 24}},
        "structure_28": {"overrides": {"structure_lookback": 28}},
        "structure_32": {"overrides": {"structure_lookback": 32}},
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate structure lookback on top of the paired ICT NY-only slow-recovery frontier."
    )
    ...
    verdict = "SLOW_RECOVERY_BASE_REMAINS_STRUCTURE_FRONTIER"
    interpretation = (
        "The slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby structure-lookback changes do not beat the current frontier."
    )
    if best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_STRUCTURE_EXTENSION_IDENTIFIED_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "At least one structure-lookback calibration improves the slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_STRUCTURE_SURVIVOR_BUT_NOT_FRONTIER_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "A structure-lookback calibration survives on top of the slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_STRUCTURE_EXTENSION_ONLY_ON_SLOW_RECOVERY_BASE"
        interpretation = (
            "A positive structure-lookback calibration exists on top of the slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_calibration",
        ...
        "best_robust_structure_variant": best_robust_variant,
