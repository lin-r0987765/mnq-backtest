...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_close_recovery_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_rejection_close_ratio": 0.0}},
        "fvg_close_recovery_0p25": {"overrides": {"fvg_rejection_close_ratio": 0.25}},
        "fvg_close_recovery_0p50": {"overrides": {"fvg_rejection_close_ratio": 0.50}},
        "fvg_close_recovery_0p75": {"overrides": {"fvg_rejection_close_ratio": 0.75}},
        "fvg_close_recovery_1p00": {"overrides": {"fvg_rejection_close_ratio": 1.00}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_rejection_close_ratio": float(payload["params"]["fvg_rejection_close_ratio"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "ifvg_entries": int(payload["metadata"].get("ifvg_entries", 0)),
                "fvg_close_filtered_retests": int(payload["metadata"].get("fvg_close_filtered_retests", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG close-back recovery strength on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "fvg_rejection_close_ratio": float(params["fvg_rejection_close_ratio"]),
            ...
            "metadata": {
                ...
                "fvg_close_filtered_retests": int(signals.metadata.get("fvg_close_filtered_retests", 0)),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_CLOSE_RECOVERY_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG close-recovery filters do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_CLOSE_RECOVERY_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG close-recovery variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_CLOSE_RECOVERY_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG close-recovery calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_FVG_CLOSE_RECOVERY_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An FVG close-recovery calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_CLOSE_RECOVERY_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG close-recovery calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_close_recovery_calibration",
        ...
        "best_robust_fvg_close_recovery_variant": best_robust_variant,
