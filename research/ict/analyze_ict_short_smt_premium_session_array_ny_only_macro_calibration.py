...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_macro_calibration.json"
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate macro timing on top of the paired ICT NY-only short-SMT premium-plus-session-array frontier."
    )
    ...
    verdict = "NY_ONLY_BASE_REMAINS_MACRO_FRONTIER"
    interpretation = (
        "The NY-only short-SMT premium-plus-session-array paired ICT base remains the strongest robust profile. Macro timing still acts as a thinning filter rather than a robust extension on this stronger frontier."
    )
    if best_robust_macro and best_robust_macro["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_MACRO_EXTENSION_IDENTIFIED_ON_NY_ONLY_BASE"
        interpretation = (
            "At least one macro timing calibration improves the NY-only short-SMT premium-plus-session-array base while preserving most of its activity."
        )
    elif best_robust_macro and best_robust_macro["total_return_pct"] > 0:
        verdict = "ROBUST_MACRO_SURVIVOR_BUT_NOT_FRONTIER_ON_NY_ONLY_BASE"
        interpretation = (
            "A macro timing calibration survives on top of the NY-only short-SMT premium-plus-session-array base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_MACRO_EXTENSION_ONLY_ON_NY_ONLY_BASE"
        interpretation = (
            "A positive macro timing calibration exists on top of the NY-only short-SMT premium-plus-session-array base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_macro_calibration",
