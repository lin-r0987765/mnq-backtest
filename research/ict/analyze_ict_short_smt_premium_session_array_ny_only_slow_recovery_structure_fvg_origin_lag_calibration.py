...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_origin_lag_calibration.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "structure_frontier_base": {"overrides": {"fvg_origin_max_lag_bars": 0}},
        "fvg_origin_lag_1": {"overrides": {"fvg_origin_max_lag_bars": 1}},
        "fvg_origin_lag_2": {"overrides": {"fvg_origin_max_lag_bars": 2}},
        "fvg_origin_lag_3": {"overrides": {"fvg_origin_max_lag_bars": 3}},
        "fvg_origin_lag_4": {"overrides": {"fvg_origin_max_lag_bars": 4}},
        "fvg_origin_lag_5": {"overrides": {"fvg_origin_max_lag_bars": 5}},
    }
        ...
        ranked.append(
            {
                ...
                "fvg_origin_max_lag_bars": int(payload["params"]["fvg_origin_max_lag_bars"]),
                "fvg_entries": int(payload["metadata"].get("fvg_entries", 0)),
                "ifvg_entries": int(payload["metadata"].get("ifvg_entries", 0)),
                "fvg_origin_lag_filtered_shifts": int(payload["metadata"].get("fvg_origin_lag_filtered_shifts", 0)),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate FVG origin-lag gating on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "fvg_origin_max_lag_bars": int(params["fvg_origin_max_lag_bars"]),
            ...
            "metadata": {
                ...
                "fvg_origin_lag_filtered_shifts": int(
                    signals.metadata.get("fvg_origin_lag_filtered_shifts", 0)
                ),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_FVG_ORIGIN_LAG_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby FVG origin-lag variants do not beat the current frontier."
    )
    if all_variants_match_base:
        verdict = "FVG_ORIGIN_LAG_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested FVG origin-lag variants collapse to the same structure-aware slow-recovery NY-only frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_FVG_ORIGIN_LAG_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one FVG origin-lag calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
        ...
        interpretation = (
            "An FVG origin-lag calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_FVG_ORIGIN_LAG_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive FVG origin-lag calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_fvg_origin_lag_calibration",
        ...
        "best_robust_fvg_origin_lag_variant": best_robust_variant,
