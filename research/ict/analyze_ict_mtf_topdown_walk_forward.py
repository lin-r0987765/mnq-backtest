...
DEFAULT_OUTPUT = (
    ...
    / "ict_mtf_topdown_walk_forward.json"
)
RESEARCH_STANDARD = {
...
def _segment(result) -> dict[str, Any]:
...
def _resolve_output_path(requested_output: str, *, profile_name: str) -> Path:
    ...
    if profile_name == "mtf_topdown_continuation_execution_candidate":
        return requested_path.with_name("ict_mtf_topdown_execution_candidate_walk_forward.json")
    if profile_name == "mtf_topdown_continuation_setup_execution_candidate":
        return requested_path.with_name("ict_mtf_topdown_setup_execution_candidate_walk_forward.json")
    if profile_name == "mtf_topdown_continuation_timing_candidate":
        return requested_path.with_name("ict_mtf_topdown_timing_candidate_walk_forward.json")
    if profile_name == "mtf_topdown_continuation_regularized_long_only_candidate":
        return requested_path.with_name(
            "ict_mtf_topdown_regularized_long_only_candidate_walk_forward.json"
        )
    if profile_name == "mtf_topdown_continuation_regularized_long_only_am_candidate":
        return requested_path.with_name(
            "ict_mtf_topdown_regularized_long_only_am_candidate_walk_forward.json"
        )
    if profile_name == "mtf_topdown_continuation_quality_candidate":
        return requested_path.with_name("ict_mtf_topdown_quality_candidate_walk_forward.json")
    return requested_path.with_name(f"{profile_name}_walk_forward.json")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run walk-forward evaluation for the MTF ICT top-down continuation branch."
    )
    ...
    for fold_number, window in enumerate(windows, start=1):
        ...
        folds.append(
            {
                "fold": fold_number,
                "train": _segment(train_result),
                "validation": _segment(validation_result),
                "holdout": _segment(holdout_result),
            }
        )

    output = {
        "analysis": "ict_mtf_topdown_walk_forward",
