...
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_frontier_sweep_threshold.json"
)


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "lite_frontier_base": {"overrides": {"liq_sweep_threshold": 0.0006}},
        "sweep_0p0003": {"overrides": {"liq_sweep_threshold": 0.0003}},
        "sweep_0p0004": {"overrides": {"liq_sweep_threshold": 0.0004}},
        "sweep_0p0005": {"overrides": {"liq_sweep_threshold": 0.0005}},
        "sweep_0p0007": {"overrides": {"liq_sweep_threshold": 0.0007}},
        "sweep_0p0008": {"overrides": {"liq_sweep_threshold": 0.0008}},
    }
        ...
        ranked.append(
            {
                ...
                "ifvg_entries": int(payload["metadata"]["ifvg_entries"]),
                "bullish_sweeps": int(payload["metadata"]["bullish_sweeps"]),
                "bearish_sweeps": int(payload["metadata"]["bearish_sweeps"]),
            }
        )
    ranked.sort(
        key=lambda row: (
...
def _verdict(
    *,
    ...
    if best_robust_variant is not None:
        return (
            "ROBUST_LITE_FRONTIER_SWEEP_EXTENSION_IDENTIFIED",
            "At least one sweep-threshold variant improves both activity and return versus the active lite frontier.",
        )
    if best_non_base is None:
        return (
            "LITE_FRONTIER_SWEEP_NO_POSITIVE_VARIANTS",
            "No positive sweep-threshold variant survives on top of the active lite frontier.",
        )
    if best_positive_density_variant is not None and best_positive_density_variant["total_trades"] > base_trades:
        return (
            "LITE_FRONTIER_SWEEP_DENSITY_EXTENSION_ONLY",
            "A sweep-threshold variant improves density on the active lite frontier, but not enough to replace the base cleanly.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_FRONTIER_SWEEP_SURVIVOR_BUT_NOT_EXTENSION",
            "Positive sweep-threshold variants survive on the active lite frontier, but none improve it enough to promote.",
        )
    return (
        "LITE_FRONTIER_SWEEP_CALIBRATION_REJECTED",
        "The tested sweep-threshold variants do not improve the active lite frontier.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run sweep-threshold recalibration directly on the active lite ICT frontier."
    )
    ...
    output = {
        "analysis": "ict_lite_frontier_sweep_threshold",
