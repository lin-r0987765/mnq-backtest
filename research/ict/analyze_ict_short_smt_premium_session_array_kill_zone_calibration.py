...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_kill_zone_calibration.json"
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate kill-zone reintroduction on top of the paired ICT short-SMT premium-plus-session-array frontier."
    )
    ...
    verdict = "SESSION_ARRAY_PREMIUM_BASE_REMAINS_KILL_ZONE_FRONTIER"
    interpretation = (
        "The short-SMT premium-plus-session-array paired ICT base remains the strongest robust profile. Kill-zone timing still acts as a thinning filter rather than a robust extension on the stronger frontier."
    )
    if best_robust_kill_zone and best_robust_kill_zone["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_KILL_ZONE_EXTENSION_IDENTIFIED_ON_SESSION_ARRAY_BASE"
        interpretation = (
            "At least one kill-zone calibration improves the short-SMT premium-plus-session-array base while preserving most of its activity."
        )
    elif best_robust_kill_zone and best_robust_kill_zone["total_return_pct"] > 0:
        verdict = "ROBUST_KILL_ZONE_SURVIVOR_BUT_NOT_FRONTIER_ON_SESSION_ARRAY_BASE"
        interpretation = (
            "A kill-zone calibration survives on top of the short-SMT premium-plus-session-array base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_KILL_ZONE_EXTENSION_ONLY_ON_SESSION_ARRAY_BASE"
        interpretation = (
            "A positive kill-zone calibration exists on top of the short-SMT premium-plus-session-array base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_kill_zone_calibration",
