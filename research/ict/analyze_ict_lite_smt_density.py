...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_smt_density.json"


def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        "lite_base": {"enable_smt": True, "overrides": {}},
        "lite_smt_lookback_6": {"enable_smt": True, "overrides": {"smt_lookback": 6}},
        "lite_smt_lookback_8": {"enable_smt": True, "overrides": {"smt_lookback": 8}},
        "lite_smt_lookback_12": {"enable_smt": True, "overrides": {"smt_lookback": 12}},
        "lite_smt_threshold_0p0015": {"enable_smt": True, "overrides": {"smt_threshold": 0.0015}},
        "lite_smt_lookback_8_threshold_0p0015": {
            "enable_smt": True,
            "overrides": {"smt_lookback": 8, "smt_threshold": 0.0015},
        },
        "lite_smt_off_control": {"enable_smt": False, "overrides": {}},
    }
        ...
        ranked.append(
            {
                ...
                "trade_gain_vs_base": trades - base_trades,
    ...
    *,
    ...
    if best_robust_variant is not None:
        return (
            ...
            "At least one SMT density variant improves both activity and return versus the lite reversal baseline.",
        )
    if best_non_base is None:
        return (
            "LITE_SMT_DENSITY_NO_POSITIVE_VARIANTS",
            "No positive SMT variant survived on top of the lite reversal baseline.",
        )
    if best_positive_density_variant and best_positive_density_variant["total_trades"] > base_trades:
        return (
            "LITE_SMT_DENSITY_EXTENSION_ONLY",
            "An SMT density variant materially improves activity on the lite reversal baseline, but it does not improve return enough to replace the base cleanly.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_SMT_SURVIVOR_BUT_NOT_DENSITY_EXTENSION",
            "Positive SMT variants survive on the lite reversal baseline, but none improve density enough to change the roadmap.",
        )
    return (
        "LITE_SMT_CALIBRATION_REJECTED",
        "The tested SMT density variants do not improve the lite reversal baseline.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate SMT density on top of the lite ICT reversal baseline."
    )
