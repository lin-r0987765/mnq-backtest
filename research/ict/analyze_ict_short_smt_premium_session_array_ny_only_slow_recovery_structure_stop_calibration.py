...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_stop_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "stop_atr_1p5": {"overrides": {"stop_loss_atr_mult": 1.5}},
        "stop_atr_1p75": {"overrides": {"stop_loss_atr_mult": 1.75}},
        "stop_atr_2p25": {"overrides": {"stop_loss_atr_mult": 2.25}},
        "stop_atr_2p5": {"overrides": {"stop_loss_atr_mult": 2.5}},
        "stop_atr_3p0": {"overrides": {"stop_loss_atr_mult": 3.0}},
    }
        ...
        ranked.append(
            {
                ...
                "stop_loss_atr_mult": float(payload["params"]["stop_loss_atr_mult"]),
                "avg_trade_pct": float(metrics["avg_trade_pct"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate stop-loss ATR multiple on top of the paired ICT structure-aware frontier."
    )
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_STOP_BASELINE"
    interpretation = (
        "The structure-aware frontier remains strongest at the current stop-loss ATR multiple. Nearby stop changes do not beat the current base."
    )
    if all_variants_match_base:
        verdict = "STOP_CALIBRATION_PLATEAU_ON_STRUCTURE_BASE"
        interpretation = (
            "All tested stop-loss ATR variants collapse to the same structure-aware frontier result, indicating a plateau rather than a new edge."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_STOP_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one stop-loss ATR calibration improves the structure-aware frontier while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_STOP_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "A stop-loss ATR calibration survives on top of the structure-aware frontier without collapsing activity, but it does not beat the base."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_STOP_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive stop-loss ATR calibration exists on top of the structure-aware frontier, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_stop_calibration",
        ...
        "best_robust_stop_variant": best_robust_variant,
