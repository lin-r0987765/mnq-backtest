...
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_short_smt_premium_daily_bias_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "daily_bias_short_lookback_2": {
            "overrides": {
                "use_daily_bias_filter": True,
                "daily_bias_lookback": 2,
            },
        },
        "daily_bias_longer_lookback_5": {
            "overrides": {
                "use_daily_bias_filter": True,
                "daily_bias_lookback": 5,
            },
        },
        "daily_bias_softer_thresholds": {
            "overrides": {
                "use_daily_bias_filter": True,
                "daily_bias_bull_threshold": 0.55,
                "daily_bias_bear_threshold": 0.45,
            },
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate daily-bias reintroduction on top of the paired ICT short-SMT premium base."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                ...
                "daily_bias_lookback": int(params["daily_bias_lookback"]),
                "daily_bias_bull_threshold": float(params["daily_bias_bull_threshold"]),
                "daily_bias_bear_threshold": float(params["daily_bias_bear_threshold"]),
            },
    ...
    verdict = "PREMIUM_BASE_REMAINS_DAILY_BIAS_FRONTIER"
    interpretation = (
        "The short-SMT premium paired ICT base remains the strongest robust profile. Daily bias still acts as an over-restrictive narrative filter rather than a robust extension."
    )
    if best_robust_daily_bias and best_robust_daily_bias["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_DAILY_BIAS_EXTENSION_IDENTIFIED"
        interpretation = (
            "At least one daily-bias calibration improves the short-SMT premium base while preserving most of its activity. The next step should preserve that daily-bias survivor before reopening additional context."
        )
    elif best_robust_daily_bias and best_robust_daily_bias["total_return_pct"] > 0:
        verdict = "ROBUST_DAILY_BIAS_SURVIVOR_BUT_NOT_FRONTIER"
        interpretation = (
            "A daily-bias calibration survives on top of the short-SMT premium base without collapsing activity, but it does not beat the base itself. The next step should keep premium as the frontier and treat daily bias as a secondary optional branch."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_DAILY_BIAS_EXTENSION_ONLY"
        interpretation = (
            "A positive daily-bias calibration exists on top of the short-SMT premium base, but only as a thinner continuation. The next step should treat daily bias as optional refinement rather than a new frontier."
        )

    output = {
        "analysis": "ict_short_smt_premium_daily_bias_calibration",
        ...
        "best_robust_daily_bias_variant": best_robust_daily_bias,
