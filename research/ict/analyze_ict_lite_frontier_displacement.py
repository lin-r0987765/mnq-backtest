...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "qc_regime_prototypes" / "ict_lite_frontier_displacement.json"
        ...
        ranked.append(
            {
                ...
                "displacement_filtered_shifts": int(payload["metadata"]["displacement_filtered_shifts"]),
...
def _verdict(
    *,
    ...
    if best_robust_variant is not None:
        return (
            "ROBUST_LITE_FRONTIER_DISPLACEMENT_EXTENSION_IDENTIFIED",
            "At least one displacement-body variant improves both activity and return versus the active lite frontier.",
        )
    if best_non_base is None:
        return (
            "LITE_FRONTIER_DISPLACEMENT_NO_POSITIVE_VARIANTS",
            "No positive displacement-body variant survives on top of the active lite frontier.",
        )
    if best_positive_density_variant is not None and best_positive_density_variant["total_trades"] > base_trades:
        return (
            "LITE_FRONTIER_DISPLACEMENT_DENSITY_EXTENSION_ONLY",
            "A displacement-body variant improves density on the active lite frontier, but not enough to replace the base cleanly.",
        )
    if best_non_base["total_return_pct"] > 0:
        return (
            "LITE_FRONTIER_DISPLACEMENT_SURVIVOR_BUT_NOT_EXTENSION",
            "Positive displacement-body variants survive on the active lite frontier, but none improve it enough to promote.",
        )
    return (
        "LITE_FRONTIER_DISPLACEMENT_CALIBRATION_REJECTED",
        "The tested displacement-body variants do not improve the active lite frontier.",
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run displacement-body recalibration directly on the active lite ICT frontier."
    )
    ...
    output = {
        "analysis": "ict_lite_frontier_displacement",
