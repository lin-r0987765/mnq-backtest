...
DEFAULT_OUTPUT = (
    ...
    / "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_ob_lookback_calibration.json"
)
...
def _variant_specs() -> dict[str, dict[str, Any]]:
    return {
        ...
        "ob_lookback_8": {"overrides": {"ob_lookback": 8}},
        "ob_lookback_12": {"overrides": {"ob_lookback": 12}},
        "ob_lookback_20": {"overrides": {"ob_lookback": 20}},
        "ob_lookback_24": {"overrides": {"ob_lookback": 24}},
        "ob_lookback_30": {"overrides": {"ob_lookback": 30}},
    }
        ...
        ranked.append(
            {
                ...
                "ob_lookback": int(payload["params"]["ob_lookback"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibrate order-block lookback on top of the paired ICT slow-recovery structure frontier."
    )
    ...
    for label, spec in _variant_specs().items():
        ...
        variants[label] = {
            "params": {
                "ob_lookback": int(params["ob_lookback"]),
    ...
    verdict = "STRUCTURE_FRONTIER_REMAINS_OB_LOOKBACK_FRONTIER"
    interpretation = (
        "The structure-aware slow-recovery NY-only paired ICT base remains the strongest robust profile. Nearby order-block lookback changes do not beat the current frontier."
    )
    if best_robust_variant and best_robust_variant["total_return_pct"] > base_total_return_pct:
        verdict = "ROBUST_OB_LOOKBACK_EXTENSION_IDENTIFIED_ON_STRUCTURE_BASE"
        interpretation = (
            "At least one order-block lookback calibration improves the structure-aware slow-recovery NY-only paired ICT base while preserving most of its activity."
        )
    elif best_robust_variant and best_robust_variant["total_return_pct"] > 0:
        verdict = "ROBUST_OB_LOOKBACK_SURVIVOR_BUT_NOT_FRONTIER_ON_STRUCTURE_BASE"
        interpretation = (
            "An order-block lookback calibration survives on top of the structure-aware slow-recovery NY-only paired ICT base without collapsing activity, but it does not beat the frontier."
        )
    elif best_non_base and best_non_base["total_trades"] > 0 and best_non_base["total_return_pct"] > 0:
        verdict = "THIN_OB_LOOKBACK_EXTENSION_ONLY_ON_STRUCTURE_BASE"
        interpretation = (
            "A positive order-block lookback calibration exists on top of the structure-aware slow-recovery NY-only paired ICT base, but only as a thinner continuation."
        )

    output = {
        "analysis": "ict_short_smt_premium_session_array_ny_only_slow_recovery_structure_ob_lookback_calibration",
        ...
        "best_robust_ob_lookback_variant": best_robust_variant,
