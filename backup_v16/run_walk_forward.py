...
def walk_forward_combined(df, engine, train_days=20, test_days=10, step_days=10):
    """Walk-forward for combined portfolio (ORB + VWAP 50/50)."""
    dates = sorted(set(df.index.date))
    n_days = len(dates)
    ...
    while start + train_days + test_days <= n_days:
        ...
        try:
            ...
            combo_eq = combine_equity_wf(orb_r.equity_curve, vwap_r.equity_curve, 0.5, 0.5)
...
def print_combined_wf_summary(folds):
    ...
    print(f"\n{'='*80}")
    print(f"Walk-Forward Validation: Combined Portfolio (50/50)")
    print(f"{'='*80}")
