...
def combine_equity_active_wf(
    ...
    arr1 = np.array(orb_result.equity_curve)
    arr2 = np.array(vwap_result.equity_curve)
    mask1 = _position_mask_from_result(orb_result)
    mask2 = _position_mask_from_result(vwap_result)
    n = min(len(arr1), len(arr2), len(mask1), len(mask2))
    if n < 2:
        return arr1[:n] if len(arr1) > 0 else arr2[:n]
...
def walk_forward_combined(df, engine, train_days=20, test_days=10, step_days=10):
    ...
    while start + train_days + test_days <= n_days:
        ...
        try:
            ...
            combo_eq = combine_equity_active_wf(orb_r, vwap_r)
...
def main():
    ...
    all_results = {
        "config": {
            ...
            "step_days": STEP_DAYS,
            "total_days": n_days,
            "combined_active_reuse_weight": ACTIVE_REUSE_WEIGHT,
        },
