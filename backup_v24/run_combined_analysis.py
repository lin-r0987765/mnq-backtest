...
def main():
    ...
    for w_orb, w_vwap in allocations:
        combined_eq = combine_equity_static(orb_result.equity_curve, vwap_result.equity_curve, w_orb, w_vwap)
    ...
    active_reuse_eq = combine_results_active_reuse(orb_result, vwap_result, active_weight=DEFAULT_ACTIVE_REUSE_WEIGHT)
    ...
    if len(orb_oos.equity_curve) > 0 and len(vwap_oos.equity_curve) > 0:
        ...
        combined_oos_active = combine_results_active_reuse(orb_oos, vwap_oos, active_weight=DEFAULT_ACTIVE_REUSE_WEIGHT)
